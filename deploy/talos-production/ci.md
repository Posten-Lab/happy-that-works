...
The selector compares against the last successful delivered commit, so failed native changes remain pending. Documentation/server-only changes can skip delivery; native inputs require a binary. The app uses Expo's fingerprint runtime policy: each native fingerprint is its own OTA runtime. OTA requires a finished production iOS store binary whose runtime and fingerprint both equal the current hash. Old runtimes and a full history page no longer block a verified current runtime. Missing or conflicting evidence still prevents OTA; auto mode builds a binary and forced OTA fails.

Builds 15–19 used the legacy literal `talos-1`. They cannot be relabelled after installation. Migration therefore requires one native build using the fingerprint policy; subsequent compatible JavaScript changes can use OTA. No updates are published to `talos-1` by the new pipeline. Before submission, the finished build's runtime/fingerprint and IPA runtime must match the fingerprint calculated before building.

Apple submission configuration is written only after the binary has been built and verified. `submit-ios.sh` restores the exact original `eas.json` bytes after success, failure or termination. Temporary key paths never participate in build fingerprinting. Jenkins archives the computed fingerprint, runtime and compatibility decision for diagnosis, alongside the existing sanitized delivery receipts.
