...
export class Encryption {
    ...
    static async create(masterSecret: Uint8Array) {
        ...
        const contentDataKey = await deriveKey(masterSecret, encryptionContexts.content, ['content']);

        // Derive content data key keypair
        const contentKeyPair = sodium.crypto_box_seed_keypair(contentDataKey);

        // Derive anonymous ID
        const anonID = encodeHex((await deriveKey(masterSecret, encryptionContexts.analytics, ['analytics', 'id']))).slice(0, 16).toLowerCase();
    ...
    async initializeSessions(sessions: Map<string, Uint8Array | null>): Promise<void> {
        for (const [sessionId, dataKey] of sessions) {
            ...
            const blobKey = dataKey
                ? await deriveKey(dataKey, encryptionContexts.blobs, ['session'])
                : this.masterBlobKey;
