...
const storageUrl = 'https://files.cluster-fluster.com/talos/session-1/ref?X-Amz-Signature=s3-secret&policy=secret-policy';
...
describe('requestAttachmentUpload', () => {
    it('classifies request-upload HTTP failures against the Talos API host', async () => {
    ...
    it('classifies request-upload network failures against the Talos API host', async () => {
    ...
    it('classifies request-upload response parse failures without leaking credentials or refs', async () => {
        fetchMock.mockResolvedValueOnce(response({
            ok: true,
            jsonError: new Error(`Unexpected token at ${apiBlobUrl} Bearer ${credentials.token} ref talos/session-1/ref`),
        }));
...
describe('uploadEncryptedBlob', () => {
    ...
    it('classifies PUT blob upload network failures on the Talos API host without leaking query data', async () => {
        fetchMock.mockRejectedValueOnce(new Error('Failed to fetch'));
...
describe('downloadEncryptedAttachment', () => {
    ...
    it('classifies request-download response parse failures without leaking attachment refs', async () => {
        fetchMock.mockResolvedValueOnce(response({
            ok: true,
            jsonError: new Error(`Invalid JSON for ref talos/session-1/ref at ${apiBlobUrl}`),
        }));
    ...
    it('classifies blob-download body read failures against storage without leaking presigned URL data', async () => {
        fetchMock
            ...
            .mockResolvedValueOnce(response({
                ok: true,
                arrayBufferError: new Error(`stream reset for ${storageUrl} ref talos/session-1/ref`),
            }));
