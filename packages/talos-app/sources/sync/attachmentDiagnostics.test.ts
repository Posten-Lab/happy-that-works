...
describe('attachment diagnostic serialization', () => {
    ...
    it('sanitizes unsafe messages and reasons before serialization', () => {
        const diagnostic = createAttachmentDiagnostic({
            ...
            url: 'https://files.cluster-fluster.com/talos/ref?X-Amz-Signature=secret',
            serverUrl: 'https://api.cluster-fluster.com',
            message: 'Fetch failed for https://files.cluster-fluster.com/talos/ref?X-Amz-Signature=secret&policy=secret using Bearer secret-token',
            reason: 'Read failed from file:///Users/devdvlive/Projects/talos/local.bin and /Users/devdvlive/Projects/talos/other.bin',
        });
    ...
    it('redacts non-http absolute URLs in diagnostic text', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            method: 'GET',
            message: 'Fetch failed for s3://bucket/talos/ref?X-Amz-Signature=secret',
    ...
    it('redacts desktop local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Failed to fetch',
            reason: 'Read failed from /Users/devdvlive/My Projects/talos/blob.bin',
        });
        ...
        expect(serialized).not.toContain('talos/blob.bin');
        expect(serialized).not.toContain('blob.bin');
    });

    it('redacts file URI local paths with spaces in path segments', () => {
        const source = 'Read failed from file:///Users/devdvlive/My Projects/talos/blob.bin';
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: source,
        });
        ...
        for (const rendered of [serialized, message]) {
            ...
            expect(rendered).not.toContain('Projects/talos');
    ...
    it('redacts Windows desktop local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: 'Read failed from C:\\Users\\devdvlive\\My Projects\\talos\\blob.bin',
        });
        ...
        expect(serialized).not.toContain('My Projects');
        expect(serialized).not.toContain('Projects\\talos');
        expect(serialized).not.toContain('blob.bin');
    });
    ...
    it('redacts forward-slash Windows local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: 'Read failed from C:/Users/devdvlive/My Projects/talos/blob.bin',
        });
    ...
    it('redacts assignment-prefixed local paths before diagnostic serialization', () => {
        const sources = [
            ...
            'path=/data/user/0/com.talos/cache/blob.bin',
        ...
        for (const source of sources) {
            ...
            expect(rendered).not.toContain('C:/Users');
            expect(rendered).not.toContain('My Projects');
            expect(rendered).not.toContain('com.talos');
            expect(rendered).not.toContain('blob.bin');
        }
    });
    ...
    it('redacts mobile local paths in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Read failed from /data/user/0/com.talos/cache/blob.bin',
    ...
    it('redacts Android app data paths in reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: 'Read failed from /data/data/com.talos/cache/blob.bin',
        });

        const serialized = JSON.stringify(diagnostic);
        expect(serialized).toContain('[local-file]');
        expect(serialized).not.toContain('/data/data');
        expect(serialized).not.toContain('com.talos');
        expect(serialized).not.toContain('blob.bin');
    });
    ...
    it('sanitizes dirty whitelisted fields when formatting diagnostics for logs', () => {
        const rendered = formatAttachmentDiagnosticForLog({
            leg: 'blob-download',
            method: 'GET',
            host: 'https://files.cluster-fluster.com/talos/ref?X-Amz-Signature=secret',
            target: 'external-storage',
            status: 403,
            statusText: 'Forbidden from https://files.cluster-fluster.com/talos/ref?X-Amz-Signature=secret',
            message: 'Failed to fetch',
            reason: 'missing-blob-key',
        }, {
            platform: 'web talos/session-1/ref',
            client: 'web/1.2.3 Bearer secret-token /data/data/com.talos/cache/blob.bin',
        });
    ...
    it('drops dirty non-host values from host output', () => {
        const rendered = [
            'C:\\Users\\devdvlive\\My Projects\\talos\\blob.bin',
            'data:,SECRET',
            'user:secret@files.cluster-fluster.com',
            'file:///Users/devdvlive/My Projects/talos/blob.bin',
    ...
    it('redacts bare attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Render failed for ref: talos/session-1/ref and ref talos/session-1/ref',
    ...
    it('redacts standalone slash-bearing attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'talos/session-1/ref',
            reason: 'missing-blob-key talos/session-1/ref',
        });
    ...
    it('redacts leading-slash attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'missing /talos/session-1/ref?X-Amz-Signature=secret',
            reason: 'ref=/talos/session-1/ref',
        });

        const serialized = JSON.stringify(diagnostic);
        expect(serialized).toContain('[attachment-ref]');
        expect(serialized).not.toContain('/talos/session-1/ref');
        expect(serialized).not.toContain('ref=/talos/session-1/ref');
        expect(serialized).not.toContain('X-Amz-Signature');
    });
...
describe('AttachmentDiagnosticError', () => {
    ...
    it('sanitizes unsafe wrapper error messages', () => {
        const error = createAttachmentDiagnosticError(
            'Blob upload failed for https://files.cluster-fluster.com/talos/ref?X-Amz-Signature=secret using Bearer secret-token at /data/user/0/com.talos/cache/blob.bin',
            {
                leg: 'blob-upload',
                method: 'POST',
                url: 'https://files.cluster-fluster.com/talos/ref?X-Amz-Signature=secret',
