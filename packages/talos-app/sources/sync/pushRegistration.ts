...
export async function syncCurrentPushToken(credentials: AuthCredentials): Promise<SyncCurrentPushTokenResult> {
    ...
    const projectId = getExpoProjectId();
    if (!projectId) {
        // Local installations cannot deliver Expo push notifications. Ask for
        // permission only after the operator configures the Talos project.
        return {
            registered: false,
...
export function getCurrentPushDeviceMetadata(): CurrentPushDeviceMetadata {
    ...
    const appParts = [
        Application.nativeApplicationVersion ? `Talos ${Application.nativeApplicationVersion}` : null,
