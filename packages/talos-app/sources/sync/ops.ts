...
export interface SpawnSessionOptions {
    ...
    /**
     * If set, the daemon spawns Codex with `--resume <id>` so the new Talos
     * session attaches to an app-server thread created by fork / duplicate.
     */
    resumeCodexThreadId?: string;
    /** Talos session id this fork was branched from (lineage). */
    parentSessionId?: string;
    /** Talos message id used as the rewind point (only set for "duplicate"). */
 ...
 * the new Claude session UUID. Caller then spawns a fresh Talos session
 * with `resumeClaudeSessionId` set to that UUID to attach a new Talos
 ...
 *   2. Spawn a new Talos session on the same machine with
 ...
 * RPC into env vars, then into the new Talos session's metadata at start
