...
const ATTACHMENT_DIAGNOSTIC_ERROR_BRAND: unique symbol = Symbol('talos.attachment-diagnostic-error');
...
function sanitizeDiagnosticText(value: string | undefined): string | undefined {
    ...
    return value
        ...
        .replace(/(^|[\s(["'])\/?(?:talos|sessions)\/[A-Za-z0-9._~@-]+(?:\/[A-Za-z0-9._~@-]+)+(?:\?[^\s"'<>]*)?/gi, '$1[attachment-ref]');
}
