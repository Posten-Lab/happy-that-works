...
export const MainView = React.memo(({ variant }: MainViewProps) => {
    ...
    if (isTablet) {
        if (sessionListViewData?.length === 0) return <EmptyMainScreen />;
        return (
            <View style={[styles.emptyStateContentContainer, { alignItems: 'center', justifyContent: 'center', gap: 28 }]}>
                <TalosBrand size={56} />
                <RoundButton title={t('sidebar.newSession')} onPress={handleNewSession} />
            </View>
        );
    }
