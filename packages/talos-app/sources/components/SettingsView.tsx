...
export const SettingsView = React.memo(function SettingsView() {
    ...
    const handleGitHub = async () => {
        if (brand.repositoryUrl) await openExternalUrl(brand.repositoryUrl);
    };

    const handleReportIssue = async () => {
        if (brand.supportUrl) await openExternalUrl(brand.supportUrl);
    };
    ...
    // GitHub connection
    const [connectingGitHub, connectGitHub] = useTalosAction(async () => {
        const params = await getGitHubOAuthParams(auth.credentials!);
        await openExternalUrl(params.url);
    });

    // GitHub disconnection
    const [disconnectingGitHub, handleDisconnectGitHub] = useTalosAction(async () => {
    ...
    const [connectingAnthropic, connectAnthropic] = useTalosAction(async () => {
        router.push('/settings/connect/claude');
    });

    // Anthropic disconnection
    const [disconnectingAnthropic, handleDisconnectAnthropic] = useTalosAction(async () => {
        const confirmed = await Modal.confirm(
    ...
    return (
        ...
        <ItemList style={{ paddingTop: 0 }}>
            ...
            <View style={{ maxWidth: layout.maxWidth, alignSelf: 'center', width: '100%' }}>
                <View style={{ alignItems: 'center', paddingVertical: 24, backgroundColor: theme.colors.surface, marginTop: 16, borderRadius: 12, marginHorizontal: 16 }}>
                    {profile.firstName ? (
                        ...
                        // Product identity when the account has no public profile.
                        <>
                            <TalosBrand size={64} />
                        </>
                    )}
            ...
            {/* Support Us */}
            {(Platform.OS === 'ios' ? config.revenueCatAppleKey : Platform.OS === 'android' ? config.revenueCatGoogleKey : config.revenueCatStripeKey) && (<ItemGroup>
                <Item
                    title={t('settings.supportUs')}
                    subtitle={isPro ? t('settings.supportUsSubtitlePro') : t('settings.supportUsSubtitle')}
                    icon={<Ionicons name="heart" size={29} color={theme.colors.accent} />}
                    showChevron={false}
            ...
            <ItemGroup title={t('settings.features')}>
                ...
                <Item
                    ...
                    icon={<Ionicons name="color-palette-outline" size={29} color={theme.colors.accent} />}
            ...
            {(__DEV__ || devModeEnabled) && (
                <ItemGroup title={t('settings.developer')}>
                    <Item
                        ...
                        icon={<Ionicons name="construct-outline" size={29} color={theme.colors.accent} />}
            ...
            <ItemGroup title={t('settings.about')} footer={t('settings.aboutFooter')}>
                ...
                <Item
                    ...
                    icon={<Ionicons name="bug-outline" size={29} color={theme.colors.accent} />}
                ...
                <Item
                    ...
                    onPress={() => openExternalUrl(brand.privacyUrl!)}
                />
                )}
                {brand.termsUrl && (
                <Item
                    ...
                    onPress={() => openExternalUrl(brand.termsUrl!)}
                />
                )}
                {Platform.OS === 'ios' && (
                    <Item
