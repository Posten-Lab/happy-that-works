...
function SessionInfoContent({ session }: { session: Session }) {
    ...
    const handleCopySessionId = useCallback(async () => {
        ...
        try {
            ...
            Modal.alert(t('common.success'), t('sessionInfo.talosSessionIdCopied'));
        } catch (error) {
    ...
    // Use TalosAction for archiving - it handles errors automatically
    ...
    // Use TalosAction for deletion - kills session first if needed, then deletes
    const [deletingSession, performDelete] = useTalosAction(async () => {
        // Prompt for worktree cleanup before killing (needs an active machine connection)
        await maybeCleanupWorktree(session.id, session.metadata?.path, session.metadata?.machineId);
        ...
        if (!result.success) {
            throw new TalosError(result.message || t('sessionInfo.failedToDeleteSession'), false);
        }
    });
    ...
    const handleCopyUpdateCommand = useCallback(async () => {
        const updateCommand = brand.installCommand;
        if (!updateCommand) {
            Modal.alert(t('common.version'), t('sessionInfo.updateCliInstructions'));
            return;
        }
        try {
    ...
    return (
        <>
            <ItemList>
                ...
                <ItemGroup>
                    <Item
                        title={t('sessionInfo.talosSessionId')}
                    ...
                    {/* TODO: migrate to `talos resume <talos-session-id>` once it works without talos-agent auth */}
                ...
                {session.metadata && (
                    <ItemGroup title={t('sessionInfo.metadata')}>
                        ...
                        {session.metadata.talosHomeDir && (
                            <Item
                                title={t('sessionInfo.talosHome')}
                                subtitle={formatPathRelativeToHome(session.metadata.talosHomeDir, session.metadata.homeDir)}
