...
import { workflowEnabled, workflowStageLabel, type WorkflowDefinition, type WorkflowSlot } from '@ahmadposten/talos-wire';
...
import { WorkflowSetup } from '@/workflows/WorkflowSetup';
import { workflowDraft, workflowSave, workflowStepError } from '@/workflows/setup';
...
export default function WorkflowsScreen() {
    ...
    const library = useSetting('workflowLibrary'), agentLibrary = useSetting('agentLibrary');
    const agents = agentLibrary.filter(a => a.provider === 'codex');
    const machines = useAllMachines({ includeOffline: true });
    const [machineId, setMachineId] = React.useState('');
    const machine = machineId ? machines.find(m => m.id === machineId) : machines.find(m => isMachineOnline(m) && m.metadata?.workflows?.version === 1) ?? machines[0];
    ...
    const choices = [...agents, ...starterAgents];
    const unusedAgent = choices.find(a => !draft || ![...draft.planners, draft.executor, ...draft.reviewers].some(s => s.agent.id === a.id));
    ...
    React.useEffect(() => { scroll.current?.scrollTo({ y: 0, animated: false }); }, [creating, !!draft, step, launch?.id, error]);
    const startDraft = (existing?: WorkflowDefinition) => {
        ...
        setStarterAgents([]); setLaunch(null); setPicker(null); setInstructions(null);
        setDraft(existing ? structuredClone(existing) : null); setCreating(!existing);
        setStep(0); setError('');
    };
    ...
    const save = () => {
        try {
            ...
            sync.applySettings({ workflowLibrary: saved.workflowLibrary, ...(starterAgents.length ? { agentLibrary: saved.agentLibrary, expAgentLibrary: true } : {}) });
    ...
    const teamSlot = (label: string, slot: WorkflowSlot, update: (s: WorkflowSlot) => void) => <View key={label} style={styles.card}>
        ...
        {picker === label && choices.map(a => <Button key={a.id} label={`Choose ${a.name} for ${label}`} onPress={() => { update({ ...slot, agent: a as WorkflowSlot['agent'] }); setPicker(null); }} />)}
        <Text style={styles.muted}>{slot.agent.model} · {slot.agent.effort ?? 'default'} effort · {label === 'Executor' ? 'Workspace edits' : 'Read-only'}</Text>
        {starterAgents.some(a => a.id === slot.agent.id) && <>
            <Input label={`${label} agent name`} value={slot.agent.name} max={60} onChange={name => update({ ...slot, agent: { ...slot.agent, name } })} />
            <Button label={`${instructions === label ? 'Hide' : 'Edit'} ${label} instructions`} onPress={() => setInstructions(instructions === label ? null : label)} />
            {instructions === label && <Input label={`${label} instructions`} value={slot.agent.instructions} multiline onChange={value => update({ ...slot, agent: { ...slot.agent, instructions: value } })} />}
        </>}
        <Input label={`${label} assignment`} value={slot.assignment} onChange={assignment => update({ ...slot, assignment })} multiline />
    </View>;
    return <ScrollView ref={scroll} keyboardShouldPersistTaps="handled" style={{ flex: 1, backgroundColor: styles.colors.surface }} contentContainerStyle={{ padding: 20, gap: 18, width: '100%', maxWidth: 960, alignSelf: 'center', paddingBottom: 70 }}>
        ...
        {!enabled ? <View style={styles.card}><Text style={styles.text}>Workflows are experimental</Text><Text style={styles.muted}>Enable Experimental Features and Workflows in Settings → Features. Existing runs continue on their coordinator machine.</Text><Button label="Open Features" onPress={() => router.push('/settings/features')} /></View> : creating ? <WorkflowSetup machines={machines} machine={machine} agents={agentLibrary} onMachine={setMachineId} onCancel={() => { setCreating(false); setError(''); }} onContinue={(team, additions) => {
            ...
            {step === 1 && <>
                ...
                {draft.planners.length < 4 && <Button label="Add planner" disabled={!unusedAgent} onPress={() => patch({ planners: [...draft.planners, { agent: unusedAgent! as WorkflowSlot['agent'], assignment: 'Independently challenge the plan and identify missing requirements.' }] })} />}
                ...
                {draft.reviewers.length < 4 && <Button label="Add reviewer" disabled={!unusedAgent} onPress={() => patch({ reviewers: [...draft.reviewers, { agent: unusedAgent! as WorkflowSlot['agent'], assignment: 'Independently inspect the result and acceptance evidence.' }] })} />}
                {!unusedAgent && <Text style={styles.muted}>To add more participants, save additional Codex agents in Agent Library first. Each participant needs a distinct identity.</Text>}
