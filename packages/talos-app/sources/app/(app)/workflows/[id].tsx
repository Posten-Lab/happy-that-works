...
export default function WorkflowRunScreen() {
    ...
    const acting = React.useRef(false), detailSequence = React.useRef(0), scroll = React.useRef<ScrollView>(null), progress = React.useRef<ScrollView>(null);
    ...
    React.useEffect(() => { progress.current?.scrollTo({ x: Math.max(0, currentIndex * 112 - 60), animated: true }); }, [currentIndex]);
        ...
        {!run ? <View style={{ gap: 16, paddingVertical: 40, alignItems: 'center' }}><ActivityIndicator color={s.colors.accent} /><Text style={s.muted}>Connecting to your workflow…</Text><Button label="Back to workflows" variant="ghost" compact onPress={() => router.push('/workflows' as any)} /></View> : <View onLayout={event => { setContentWidth(event.nativeEvent.layout.width); runTop.current = event.nativeEvent.layout.y; }} style={{ width: '100%', flexDirection: wide ? 'row' : 'column', alignItems: 'flex-start', gap: wide ? 36 : 24 }}>
            ...
            <View style={{ backgroundColor: s.colors.surface, borderRadius: 20, borderWidth: 1, borderColor: s.colors.divider, padding: 18, gap: 14 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    ...
                    <View style={{ flex: 1 }}><Text style={{ ...s.text, ...Typography.header(), fontWeight: '600' }}>{run.status === 'complete' ? 'Your team finished the job' : run.status === 'cancelled' ? 'Run stopped' : run.status === 'paused' ? 'Paused for you' : run.status === 'needs_input' ? 'Your input is needed' : workflowStageLabel[run.stage]}</Text>
            ...
            {run.status === 'needs_input' || run.status === 'paused' ? <View style={{ gap: 12 }}>
                {canApprove ? <>
                    ...
                    <View style={{ flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}><Button primary label="Resume run" disabled={busy || run.status === 'needs_input' && !note.trim()} onPress={() => void action('resume')} /><Button label="Run controls" variant="ghost" compact onPress={() => setControls(!controls)} /></View>
                    {run.status === 'needs_input' && !note.trim() && <Text style={{ ...s.muted, fontSize: 12 }}>Add guidance above to resume.</Text>}
                </>}
            ...
            <View style={{ gap: 12 }}>
                <ScrollView ref={progress} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                    {presentation!.steps.map((step, index) => <Pressable key={step.id} accessibilityRole="button" accessibilityLabel={`Step ${index + 1}, ${step.name}, ${step.state}`} onPress={() => setChosenPane(step.kind === 'plan' ? 'Plan' : step.kind === 'execute' ? 'Work' : 'Review')} style={{ width: 104, gap: 8 }}>
