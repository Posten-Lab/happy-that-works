...
export const ja: TranslationStructure = {
    ...
    settings: {
        ...
        about: 'このアプリについて',
        aboutFooter: 'TalosはCodexとClaude Codeのモバイルクライアントです。完全なエンドツーエンド暗号化を採用し、アカウントはデバイスにのみ保存されます。Anthropicとは提携していません。',
        whatsNew: '新機能',
    ...
    errors: {
        ...
        voiceConversationLimitReached: "今月の音声会話の上限に達しました。",
    ...
    server: {
        ...
        notValidTalosServer: '有効なTalos Serverではありません',
    ...
    sessionInfo: {
        ...
        talosSessionIdCopied: 'Talos Session IDがクリップボードにコピーされました',
        failedToCopySessionId: 'Talos Session IDのコピーに失敗しました',
        ...
        updateCliInstructions: "インストール時と同じ配布元から Talos を更新してください。",
    ...
    settingsVoice: {
        ...
        byoDescription: 'Talos のデフォルトの代わりに、独自の ElevenLabs エージェントを使用します。サブスクリプション不要 — 自分の ElevenLabs アカウントで直接接続できます。エージェントには2つのクライアントツールを定義する必要があります: messageClaudeCode（コーディングエージェントにテキストを送信）と processPermissionRequest（ツール使用を許可または拒否）。セッションコンテキストは {{initialConversationContext}} 動的変数を通じて受信されます。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: 'ElevenLabs Agent ID を入力してください。空のままにすると Talos のデフォルトが使用されます。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接接続',
        bypassTokenSubtitle: 'Talos サーバーをスキップし、ElevenLabs に直接接続',
    ...
    terminal: {
        ...
        terminalRequestDescription: 'ターミナルがTalosアカウントへの接続を要求しています。これにより、ターミナルは安全にメッセージを送受信できるようになります。',
    ...
    imageUpload: {
        ...
        uploadFailedMessage: ({ count }: { count: number }) => count === 1
            ? `添付ファイルを1件アップロードできませんでした。メッセージと添付ファイルはここに残っています。送信をタップして再試行してください。`
            : `添付ファイルを${count}件アップロードできませんでした。メッセージと添付ファイルはここに残っています。送信をタップして再試行してください。`,
        sendFailedMessage: `メッセージを送信できませんでした。下書きはここに残っています。再試行してください。`,
