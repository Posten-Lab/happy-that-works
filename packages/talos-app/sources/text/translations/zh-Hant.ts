...
/**
 * Chinese (Traditional) translations for the Talos app
 * Values can be:
...
export const zhHant: TranslationStructure = {
    ...
    settings: {
        ...
        about: '關於',
        aboutFooter: 'Talos 是一個 Codex 和 Claude Code 行動用戶端。它採用端對端加密，您的帳戶僅儲存在本機裝置上。與 Anthropic 無關聯。',
        whatsNew: '更新日誌',
    ...
    errors: {
        ...
        voiceConversationLimitReached: "您已達到本月的語音對話上限。",
    ...
    server: {
        ...
        notValidTalosServer: '不是有效的 Talos 伺服器',
        changeServer: '更改伺服器',
    ...
    sessionInfo: {
        ...
        talosSessionIdCopied: 'Talos 工作階段 ID 已複製到剪貼簿',
        failedToCopySessionId: '複製 Talos 工作階段 ID 失敗',
        talosSessionId: 'Talos 工作階段 ID',
        ...
        updateCliInstructions: "請從安裝 Talos 時使用的同一來源更新。",
    ...
    settingsVoice: {
        ...
        byoTitle: '使用自己的代理',
        byoDescription: '使用您自己的 ElevenLabs 代理取代 Talos 預設代理。無需訂閱 — 直接使用您自己的 ElevenLabs 帳戶連線。您的代理必須定義兩個用戶端工具：messageClaudeCode（向編碼代理傳送文字）和 processPermissionRequest（允許或拒絕工具使用）。透過 {{initialConversationContext}} 動態變數接收工作階段上下文。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: '輸入您的 ElevenLabs Agent ID。留空則使用 Talos 預設代理。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接連線',
        bypassTokenSubtitle: '跳過 Talos 伺服器，直接連線到 ElevenLabs',
    ...
    terminal: {
        ...
        terminalRequestDescription: '有終端正在請求連線到您的 Talos 帳戶。這將允許終端安全地傳送和接收訊息。',
