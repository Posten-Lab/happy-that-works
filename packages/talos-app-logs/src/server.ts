...
// Resolve talos home dir — same pattern as talos-cli/src/configuration.ts
let talosHome: string;
if (process.env.TALOS_HOME_DIR) {
    talosHome = process.env.TALOS_HOME_DIR.replace(/^~/, homedir());
} else {
    talosHome = join(homedir(), '.talos');
}

const logsDir = join(talosHome, 'app-logs');
