...
/* ─────── Chats persistence (jotai store <-> <Talos home>/state.sqlite) ─────── */
...
const talos = {
    getState: (): Promise<TalosStateSnapshot> =>
        ipcRenderer.invoke('talos:state:get'),
    createAccount: (): Promise<TalosStateSnapshot> =>
        ipcRenderer.invoke('talos:create-account'),
    startLinkDevice: (): Promise<TalosStateSnapshot> =>
        ipcRenderer.invoke('talos:start-link-device'),
    restoreSecret: (secretKey: string): Promise<TalosStateSnapshot> =>
        ipcRenderer.invoke('talos:restore-secret', secretKey),
    cancelAuth: (): Promise<TalosStateSnapshot> =>
        ipcRenderer.invoke('talos:cancel-auth'),
    logout: (): Promise<TalosStateSnapshot> =>
        ipcRenderer.invoke('talos:logout'),
    clientStatus: (): Promise<TalosAuthenticatedClientStatus> =>
        ipcRenderer.invoke('talos:client-status'),
    onState(cb: (state: TalosStateSnapshot) => void) {
        const listener = (_: unknown, state: TalosStateSnapshot) => cb(state)
        ipcRenderer.on('talos:state', listener)
        return () => ipcRenderer.off('talos:state', listener)
    },
}
...
if (process.contextIsolated) {
    try {
        ...
        contextBridge.exposeInMainWorld('talos', talos)
