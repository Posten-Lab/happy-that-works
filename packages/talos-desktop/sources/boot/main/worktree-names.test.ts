...
describe('worktree names', () => {
    ...
    it('adds v suffixes when a project name clashes', () => {
        expect(chooseVersionedName('talos', [])).toBe('talos')
        expect(chooseVersionedName('talos', ['talos'])).toBe('talos-v2')
        expect(chooseVersionedName('talos', ['talos', 'talos-v2'])).toBe('talos-v3')
    })
