...
import talosIcon from '../../assets/talos-icon.png?asset'
import {
...
import { registerTalosIpc } from './talos-worker/host'
...
ipcMain.handle(
    ...
    async (_e, args: {
        ...
        for (let attempt = 0; attempt < WORLD_CITY_NAMES.length + 32; attempt++) {
            ...
            branchName = `talos/${projectSlug}/${name}`
...
app.whenReady().then(() => {
    electronApp.setAppUserModelId('com.ahposten.talos.desktop')
    app.dock?.setIcon(talosIcon)
