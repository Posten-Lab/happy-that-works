...
if (!parentPort) {
    throw new Error('talos worker must be started via worker_threads')
}
...
const send = (m: TalosWorkerMessage) => port.postMessage(m)

let credentials: StoredCredentials | null = null
let client: TalosAuthenticatedClient | null = null
let linkFlow: LinkFlow | null = null
let state: TalosStateSnapshot = {
    status: 'starting',
...
class TalosAuthenticatedClient {
    private constructor(
        private readonly cfg: WorkerConfig,
        private readonly auth: StoredCredentials,
        private readonly encryption: TalosEncryptionContext,
    ) {}

    static async create(
        cfg: WorkerConfig,
        auth: StoredCredentials,
    ): Promise<TalosAuthenticatedClient> {
        const secret = decodeBase64(auth.secret, 'base64url')
        const encryption = await createEncryptionContext(secret)
        return new TalosAuthenticatedClient(cfg, auth, encryption)
    }
    ...
    async request(path: string, init: RequestInit = {}): Promise<Response> {
        ...
        headers.set('X-Talos-Client', this.cfg.clientId)
        return fetch(`${this.cfg.serverUrl}${path}`, { ...init, headers })
    }
}

port.on('message', (msg: TalosWorkerRequestWithId) => {
    Promise.resolve()
        .then(() => handle(msg))
        .catch((err) => {
...
async function boot(): Promise<void> {
    try {
        ...
        client = credentials ? await TalosAuthenticatedClient.create(config, credentials) : null
        setState(snapshotForCredentials(credentials))
    } catch (err) {
        setError(`Failed to load Talos credentials: ${errString(err)}`)
    }
}

async function handle(msg: TalosWorkerRequestWithId): Promise<void> {
...
async function startLinkDevice(requestId: string): Promise<void> {
    ...
    try {
        ...
        const flow: LinkFlow = {
            keypair,
            publicKey,
            authUrl: `talos:///account?${publicKey}`,
            startedAt: Date.now(),
            cancelled: false,
        }
        ...
        try {
            ...
            if (data.state === 'authorized') {
                if (typeof data.token !== 'string' || typeof data.response !== 'string') {
                    throw new Error('Talos auth response is missing token data')
                }
                const encrypted = decodeBase64(data.response)
                const secret = decryptBox(encrypted, flow.keypair.secretKey)
                if (!secret || secret.length !== 32) {
                    throw new Error('Failed to decrypt Talos account credentials')
                }
                linkFlow = null
            ...
            if (!flow.cancelled && linkFlow === flow) {
                linkFlow = null
                setError(`Talos link failed: ${errString(err)}`)
            }
            return
        }
        await delay(1000)
...
async function storeAuthenticated(token: string, secret: Uint8Array): Promise<void> {
    ...
    client = await TalosAuthenticatedClient.create(config, credentials)
    setState(snapshotForCredentials(credentials))
}

async function createEncryptionContext(masterSecret: Uint8Array): Promise<TalosEncryptionContext> {
    const contentSeed = await deriveKey(masterSecret, encryptionContexts.content, ['content'])
    ...
    const anonId = encodeHex(
        (await deriveKey(masterSecret, encryptionContexts.analytics, ['analytics', 'id'])).slice(0, 8),
    ).toLowerCase()
    const masterBlobKey = await deriveKey(masterSecret, encryptionContexts.blobs, ['master'])
    return {
        anonId,
        contentPublicKey,
        contentSecretKey,
...
async function authGetToken(secret: Uint8Array): Promise<string> {
    ...
    if (typeof data.token !== 'string') {
        throw new Error('Talos authentication did not return a token')
    }
    return data.token
}
...
async function apiJson(path: string, body: unknown): Promise<Record<string, unknown>> {
    const response = await fetch(`${config.serverUrl}${path}`, {
        ...
        headers: {
            ...
            'X-Talos-Client': config.clientId,
        },
        body: JSON.stringify(body),
    })
    if (!response.ok) {
        const text = await response.text().catch(() => '')
        throw new Error(`Talos API ${response.status}: ${text || response.statusText}`)
    }
...
function snapshotForCredentials(auth: StoredCredentials | null): TalosStateSnapshot {
    if (!auth) {
        return {
            ...stateBase(),
            status: 'unconfigured',
...
function setState(next: TalosStateSnapshot): void {
    state = next
...
function respond(requestId: string, next: TalosStateSnapshot, value?: unknown): void {
