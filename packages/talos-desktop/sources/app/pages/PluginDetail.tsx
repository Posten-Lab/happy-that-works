...
import { talosClient, useTalosState } from '@/talos/client'
...
function TalosAuthPanel() {
    const state = useTalosState()
    ...
    if (state.status === 'starting') {
        return <small className="plugin-detail__help">Loading Talos state…</small>
    }
            ...
            <div className="plugin-detail__row">
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('logout', () => talosClient.logout())}
                        disabled={busy !== null}
                    >
                        Disconnect
                    </button>
    ...
    if (state.status === 'authenticating' && authUrl) {
        return (
            <div className="plugin-detail__row plugin-detail__row--column">
                <div className="talos-auth__qr-wrap">
                    {qrDataUrl ? (
                        <img className="talos-auth__qr" src={qrDataUrl} alt="Talos authentication QR code" />
                    ) : (
                        <div className="talos-auth__qr talos-auth__qr--empty" />
                    )}
                </div>
                <div className="talos-auth__url-row">
                    <input
                        className="plugin-detail__input talos-auth__url"
                        value={authUrl}
                        readOnly
                        spellCheck={false}
                    />
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('cancel', () => talosClient.cancelAuth())}
                        disabled={busy !== null}
                    >
                        Cancel
                    </button>
        ...
        <div className="plugin-detail__row plugin-detail__row--column">
            ...
            <div className="plugin-detail__actions">
                <button
                    type="button"
                    className="plugins-page__action plugins-page__action--primary"
                    onClick={() => run('create', () => talosClient.createAccount())}
                    disabled={busy !== null}
                >
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('link', () => talosClient.startLinkDevice())}
                    disabled={busy !== null}
                >
            ...
            <div className="talos-auth__url-row">
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('restore', () => talosClient.restoreSecret(secretKey))}
