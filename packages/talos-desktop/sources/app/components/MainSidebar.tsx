...
export function MainSidebar() {
    ...
    const removeProject = (project: Project) => {
        const confirmed = window.confirm(`Delete "${project.name}" from Talos? Files on disk are not removed.`)
        if (!confirmed) return
    ...
    return (
        <aside className="app__sidebar">
            <SidebarResizer />
            <div className="app__sidebar-brand" aria-label="Talos">
                <img src={talosMark} alt="" width={40} height={40} />
                <span>TALOS</span>
            </div>
