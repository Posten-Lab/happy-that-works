// Headless replay of `talos auth login` — bypasses the Ink UI so it can run
// without a TTY. Writes credentials to $TALOS_HOME_DIR/access.key in the
...
const SERVER = process.env.TALOS_SERVER_URL || 'http://localhost:3005';
const HOME_DIR = process.env.TALOS_HOME_DIR || path.join(process.env.HOME, '.talos');
...
async function main() {
    ...
    const authUrl = `talos://terminal?${encodeBase64Url(keypair.publicKey)}`;

    // Register the auth request
    await axios.post(`${SERVER}/v1/auth/request`, {
        publicKey: pubkeyB64,
        supportsV2: true,
    }, { headers: { 'X-Talos-Client': 'cli/e2e-headless' } });
