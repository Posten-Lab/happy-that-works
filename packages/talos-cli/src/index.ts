...
/**
 * CLI entry point for talos command
 *
...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledTalosVersion, stopDaemon } from './daemon/controlClient'
import { getLatestDaemonLog } from './ui/logger'
import { killRunawayTalosProcesses } from './daemon/doctor'
...
(async () => {
  ...
  if (!args.includes('--version')) {
    logger.debug('Starting talos CLI with args: ', process.argv)
  }
  ...
  if (subcommand === 'migrate') {
    ...
    if (args[1] === 'clean') {
      if (args.slice(2).some(a => a === '--help' || a === '-h')) {
        console.log(`
${chalk.bold('talos doctor clean')} - Kill all talos-related processes (daemon + sessions)
...
`)
      ...
      const result = await killRunawayTalosProcesses()
    ...
    // Handle "talos gemini model set <model>" command
    ...
    // Handle "talos gemini model get" command
    ...
    // Handle "talos gemini project set <project-id>" command
    if (geminiSubcommand === 'project' && args[2] === 'set' && args[3]) {
      ...
      try {
        ...
        // Try to get current user email from Talos cloud token
    ...
    // Handle "talos gemini project get" command
    if (geminiSubcommand === 'project' && args[2] === 'get') {
      try {
        ...
        if (config.googleCloudProject) {
          ...
          console.log('  talos gemini project set <your-project-id>');
          console.log('');
    ...
    // Handle "talos gemini project" (no subcommand) - show help
    if (geminiSubcommand === 'project' && !args[2]) {
      console.log('Usage: talos gemini project <command>');
      console.log('');
    ...
    console.log(chalk.yellow('Note: "talos logout" is deprecated. Use "talos auth logout" instead.\n'));
    try {
...
${chalk.bold('Usage:')}
  ...
  If you want to kill all talos related processes run
  ${chalk.cyan('talos doctor clean')}

${chalk.bold('Note:')} The daemon runs in the background and manages Claude sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('talos doctor clean')}
`)
    }
    return;
  } else {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        // Talos-specific flag to disable chrome even if default is on
      } else if (arg === '--settings') {
        // Intercept --settings flag - Talos uses this internally for session hooks
        const settingsValue = args[++i] // consume the value
        console.warn(chalk.yellow(`⚠️  Warning: --settings is used internally by Talos for session tracking.`))
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('talos')} - Your agents. Your command.
...
${chalk.bold('Usage:')}
  ...
  talos notify            Send push notification
  talos daemon            Manage background service that allows
                            to spawn new sessions away from your computer
  talos doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  talos                    Start session
  talos resume cmmij8      Resume a previous session by Talos session ID
  talos --yolo             Start with bypassing permissions
                            talos sugar for --dangerously-skip-permissions
  talos --chrome           Enable Chrome browser access for this session
  talos --no-chrome        Disable Chrome even if default is on
  talos --no-sandbox       Disable Talos sandbox for this session
  talos --js-runtime bun   Use bun instead of node to spawn Claude Code
  talos --claude-env ANTHROPIC_BASE_URL=http://127.0.0.1:3456
                           Use a custom API endpoint (e.g., claude-code-router)
  talos acp gemini         Start Gemini via generic ACP runner
  talos acp -- opencode --acp
                           Start a custom ACP command
  talos acp opencode --verbose
                           Print raw ACP backend/envelope events
  talos auth login --force Authenticate
  talos doctor             Run diagnostics

${chalk.bold('Talos supports ALL Claude options!')}
  Use any claude flag with talos as you would with claude. Our favorite:
...
async function handleNotifyCommand(args: string[]): Promise<void> {
  ...
  if (showHelp) {
    console.log(`
${chalk.bold('talos notify')} - Send notification

${chalk.bold('Usage:')}
  talos notify -p <message> [-t <title>]    Send notification with custom message and optional title
  talos notify -h, --help                   Show this help

${chalk.bold('Options:')}
  -p <message>    Notification message (required)
  -t <title>      Notification title (optional, defaults to "Talos")

${chalk.bold('Examples:')}
  talos notify -p "Deployment complete!"
  talos notify -p "System update complete" -t "Server Status"
  talos notify -t "Alert" -p "Database connection restored"
`)
    return
  }
  ...
  if (!message) {
    ...
    console.log(chalk.gray('Run "talos notify --help" for usage information.'))
    process.exit(1)
  }
  ...
  if (!credentials) {
    console.error(chalk.red('Error: Not authenticated. Please run "talos auth login" first.'))
    process.exit(1)
  }
  ...
  try {
    ...
    // Use custom title or default to "Talos"
    const notificationTitle = title || 'Talos'
