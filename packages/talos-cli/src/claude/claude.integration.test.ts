 ...
 *   - Talos MCP tool usage (`mcp__talos__change_title`)
 ...
 *   - It does not exercise Talos's separate local sandbox wrapper; instead it
...
import { startTalosServer } from './utils/startTalosServer';
...
class ClaudeQueryDriver {
    private talosServer: Awaited<ReturnType<typeof startTalosServer>> | null = null;
    ...
    async start(): Promise<void> {
        ...
        this.talosServer = await startTalosServer(fakeSessionClient);
    }

    stop(): void {
        this.talosServer?.stop();
        this.talosServer = null;
    }
...
describe.skipIf(!claudeAvailable)('Claude Integration (SDK/query)', { timeout: 180_000 }, () => {
    ...
    it('should clarify natively, resume across a model switch, use TodoWrite, and cross the project boundary with native tools', async () => {
        ...
        const execution = await driver!.runTurn({
            allowedTools: ['mcp__talos__change_title', 'TodoWrite', 'TodoRead', 'Write', 'Edit', 'Read', 'Glob', 'LS'],
            ...
            prompt: [
                ...
                'Then update the talos title so it mentions OPTION_B and reply with only DONE.',
            ].join('\n'),
        ...
        expect(execution.toolUseNames).toContain('mcp__talos__change_title');
    ...
    it('should leave the file untouched and explain the refusal when native write is explicitly disallowed', async () => {
        const denied = await driver!.runTurn({
            allowedTools: ['mcp__talos__change_title', 'Write', 'Edit', 'Read', 'LS'],
