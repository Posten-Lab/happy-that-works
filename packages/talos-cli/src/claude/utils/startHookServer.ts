 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.talos/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `talos` (fresh start) - new session created
 * - `talos --continue` - continues last session (may fork)
 * - `talos --resume` - interactive picker, then resume
 * - `talos --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple Talos processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between Talos process and Claude session.
 */
