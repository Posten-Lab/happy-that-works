/**
 * Resolves the CLAUDE_CODE_ENTRYPOINT value Talos should use when launching
 ...
 * NOT itself an SDK entrypoint. The result is that every Talos-driven session
 ...
 * filter set. "remote_mobile" matches Talos's role as a remote-mobile control
 * surface for Claude Code, is part of Claude Code's own allowlist, and keeps
 * Talos-touched sessions visible in `claude --resume`.
 *
 * If the caller already exported CLAUDE_CODE_ENTRYPOINT to something specific,
 * we respect that — the user/operator wins.
 */

export const TALOS_DEFAULT_ENTRYPOINT = 'remote_mobile' as const

export function resolveTalosEntrypoint(currentValue: string | undefined): string {
    if (currentValue && currentValue.length > 0) {
        return currentValue
    }
    return TALOS_DEFAULT_ENTRYPOINT
}
