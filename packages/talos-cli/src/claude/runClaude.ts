...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    logger.debugLargeJson('[START] Talos process started', getEnvironmentInfo());
    logger.debug(`[START] Options: startedBy=${options.startedBy}, startingMode=${options.startingMode}`);

    // Validate daemon spawn requirements - fail fast on invalid config
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --talos-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    if (!machineId) {
        console.error(`[START] No machine ID found in settings, which is unexpected since authAndSetupMachineIfNeeded should have created it. Run talos doctor to collect diagnostics.`);
        process.exit(1);
    }
    ...
    let metadata: Metadata = {
        ...
        talosToolsDir: resolve(projectPath(), 'tools', 'unpacked'),
    ...
    const reconnectSessionId = process.env.TALOS_RECONNECT_SESSION_ID;
    const reconnectKeyBase64 = process.env.TALOS_RECONNECT_ENCRYPTION_KEY;
    const reconnectVariant = process.env.TALOS_RECONNECT_ENCRYPTION_VARIANT as 'legacy' | 'dataKey' | undefined;
    const reconnectSeq = process.env.TALOS_RECONNECT_SEQ;
    const reconnectMetadataVersion = process.env.TALOS_RECONNECT_METADATA_VERSION;
    const reconnectAgentStateVersion = process.env.TALOS_RECONNECT_AGENT_STATE_VERSION;
    ...
    // Fork backfill: when this Talos session was just spawned as a fork
    // of another (TALOS_FORK_CLAUDE_SESSION_ID is set by the daemon at
    ...
    // historical messages back to the Talos client — so without an
    ...
    //
    // Skipped on reconnect (TALOS_RECONNECT_*) — that path reattaches
    // to the existing Talos session, where the server already has every
    // message it needs.
    const forkClaudeSessionId = process.env.TALOS_FORK_CLAUDE_SESSION_ID;
    if (!reconnectSessionId && forkClaudeSessionId) {
        ...
        try {
            ...
            // Bind the new Talos session to the forked Claude UUID up
    ...
    // alongside this Talos session. App-sent prompts also land in the
    ...
    // to the running Talos session. SDK-emitted assistant + tool_result
    ...
    // Start Talos MCP server
    const talosServer = await startTalosServer(session);
    logger.debug(`[START] Talos MCP server started at ${talosServer.url}`);
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: talosServer.toolNames.map(toolName => `mcp__talos__${toolName}`),
    ...
    // Stop Talos MCP server
    talosServer.stop();
    logger.debug('Stopped Talos MCP server');
