...
vi.mock('@/claude/utils/startTalosServer', () => ({
    startTalosServer: mockStartTalosServer,
}));
...
vi.mock('@/claude/utils/generateHookSettings', () => ({
    generateHookSettingsFile: vi.fn(() => '/tmp/talos-hook-settings.json'),
...
describe('runClaude remote JSONL scanner', () => {
    ...
    beforeEach(() => {
        ...
        delete process.env.TALOS_RECONNECT_SESSION_ID;
        delete process.env.TALOS_RECONNECT_ENCRYPTION_KEY;
        delete process.env.TALOS_RECONNECT_ENCRYPTION_VARIANT;
        ...
        mockStartTalosServer.mockResolvedValue({
