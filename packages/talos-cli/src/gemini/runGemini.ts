 ...
 * through Talos CLI. It manages the agent lifecycle, session state, and
 * communication with the Talos server and mobile app.
 */
...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'talos connect gemini')
  //
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from Talos cloud');
  ...
  // (assigned later after Talos server setup)
  ...
  //
  // Start Talos MCP server and create Gemini backend
  //

  const talosServer = await startTalosServer(session);
  const bridgeCommand = join(projectPath(), 'bin', 'talos-mcp.mjs');
  const mcpServers = {
    talos: {
      command: bridgeCommand,
      args: ['--url', talosServer.url]
    }
  };
    ...
    switch (msg.type) {
        ...
        if (msg.status === 'running') {
          ...
          if (errorMessage.includes('Authentication required')) {
            errorMessage = `Authentication required.\n` +
              `For Google Workspace accounts, run: talos gemini project set <project-id>\n` +
              `Or use a different Google account: talos connect gemini\n` +
      ...
      case 'tool-result':
        ...
        if (msg.toolName === 'change_title' || 
            ...
            msg.toolName === 'talos__change_title') {
    ...
    while (!shouldExit) {
      ...
      try {
        ...
        pendingChangeTitle = message.message.includes('change_title') || 
                             message.message.includes('talos__change_title');
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            // Check for authentication error (Google Workspace accounts need project ID)
              ...
              errorMsg = `Authentication required. For Google Workspace accounts, you need to set a Google Cloud Project:\n` +
                         `  talos gemini project set <your-project-id>\n` +
                         `Or use a different Google account: talos connect gemini\n` +
                         `Guide: https://goo.gle/gemini-cli-auth-docs#workspace-gca`;
            }
