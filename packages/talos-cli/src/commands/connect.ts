 ...
 * - connect codex: Store OpenAI API key in Talos cloud
 * - connect claude: Store Anthropic API key in Talos cloud
 * - connect gemini: Store Gemini API key in Talos cloud
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('talos connect')} - Connect AI vendor API keys to Talos cloud

${chalk.bold('Usage:')}
  talos connect codex        Store your Codex API key in Talos cloud
  talos connect claude       Store your Anthropic API key in Talos cloud
  talos connect gemini       Store your Gemini API key in Talos cloud
  talos connect status       Show connection status for all vendors
  talos connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in Talos cloud. This enables you to use these services through Talos
...
${chalk.bold('Notes:')} 
  • You must be authenticated with Talos first (run 'talos auth login')
  • API keys are encrypted and stored securely in Talos cloud
  • You can manage your stored keys at the Talos app settings
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to Talos cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with Talos'));
        console.log(chalk.gray('  Please run "talos auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: talos connect <vendor>'));
    console.log(chalk.gray('Example: talos connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with Talos cloud
 * This ensures the Gemini SDK uses the same account as Talos
 */
