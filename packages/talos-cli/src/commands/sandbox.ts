...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `talos sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('talos sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  talos sandbox configure      Configure sandbox settings interactively
  talos sandbox status         Show current sandbox configuration
  talos sandbox disable        Disable sandboxing
  talos sandbox help           Show this help
`);
}
