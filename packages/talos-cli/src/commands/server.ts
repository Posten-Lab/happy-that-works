...
interface TalosServerPackageArtifact {
    command: string;
    prefixArgs?: string[];
    cwd: string;
    bundled?: boolean;
...
export async function handleServerCommand(args: string[]): Promise<void> {
    ...
    const dataDir = path.join(configuration.talosHomeDir, 'server-data');
    ...
    if (!artifacts) {
        console.error(chalk.red('Could not locate talos-server.'));
        ...
        console.error(chalk.gray('    - sibling packages/talos-server/sources/standalone.ts in the monorepo'));
        console.error(chalk.gray('  Install the matching Talos server distribution alongside this CLI.'));
        process.exit(1);
    }
    ...
    const env: NodeJS.ProcessEnv = {
        ...
        TALOS_MASTER_SECRET: masterSecret,
        PORT: String(opts.port),
        HOST: opts.host,
    };
    if (staticDir) env.TALOS_STATIC_DIR = staticDir;
    env.TALOS_INJECT_HTML_CONFIG = JSON.stringify({
        serverUrl,
    ...
    if (opts.persistServerUrl) {
        ...
        // Without this the CLI's auth flow would open the configured default web application.
    ...
    console.log();
    console.log(chalk.green.bold(`✓ talos-server starting at ${serverUrl}`));
    if (staticDir) {
        console.log(chalk.green(`  Open ${serverUrl} in your browser.`));
    }
    if (opts.persistServerUrl) {
        console.log(chalk.gray('  talos CLI + daemon will use this server automatically (settings.serverUrl).'));
    }
    ...
    const exitCode = await new Promise<number>(resolve => {
        child.on('exit', code => {
            console.log(chalk.gray(`\ntalos-server exited (code ${code ?? 0})`));
            resolve(code ?? 0);
        });
    });
...
function showHelp() {
    console.log(`
${chalk.bold('talos server')} - Run Talos sync server + web app locally (self-host)

${chalk.bold('Usage:')}
  talos server [--port 3005] [--host 127.0.0.1] [--reset] [--no-persist]
...
${chalk.bold('Notes:')}
  - Stores data in ${chalk.cyan('$TALOS_HOME_DIR/server-data/')}
  ...
  - Use ${chalk.cyan('--no-persist')} to run without modifying default Talos settings
...
async function ensureSettingsWriteAllowed(opts: ServerOptions, serverUrl: string): Promise<void> {
    ...
    const message =
        `talos server will write settings.serverUrl and settings.webappUrl to ${serverUrl} ` +
        `in ${configuration.settingsFile}.`;

    if (!process.stdin.isTTY || !process.stderr.isTTY) {
        console.error(chalk.red('Refusing to modify default Talos settings from a non-interactive run.'));
 ...
 *
 * pkgroll bundles into dist/, so __dirname at runtime is .../talos-cli/dist; tools/ lives
 * at .../talos-cli/tools. In rare layouts (e.g. running un-built source via tsx from src/
 * commands/), tools/ sits at .../talos-cli/tools and __dirname is .../talos-cli/src/commands,
...
function bundledBinaryName(): string {
    return process.platform === 'win32' ? 'talos-server.exe' : 'talos-server';
}
...
/**
 * Resolves the artifacts needed to spawn talos-server.
 *
 * Order:
 *   1. talos-server package (npm-installed local server artifact)
 *   2. Legacy bundled binary at tools/server/<platform>/talos-server
 *   3. Source-mode fallback for monorepo dev: ../talos-server/sources/standalone.ts via tsx
 */
...
function resolveInstalledServerPackage(): ServerArtifacts | undefined {
    try {
        const serverPackage = require_(SERVER_PACKAGE_NAME) as {
            resolveServerArtifact?: () => TalosServerPackageArtifact | undefined;
        };
...
function findSourceStandalone(): string | undefined {
    const candidates = [
        path.resolve(__dirname, '../../../talos-server/sources/standalone.ts'),
        path.resolve(__dirname, '../../talos-server/sources/standalone.ts'),
        path.resolve(process.cwd(), 'packages/talos-server/sources/standalone.ts'),
        path.resolve(process.cwd(), '../talos-server/sources/standalone.ts'),
    ];
...
function findWebappDir(): string | undefined {
    ...
    const candidates = [
        path.resolve(__dirname, '../../../talos-app/dist'),
        path.resolve(__dirname, '../../talos-app/dist'),
        path.resolve(process.cwd(), 'packages/talos-app/dist'),
    ];
    for (const c of candidates) {
...
async function spawnAndWait(art: ServerArtifacts, env: NodeJS.ProcessEnv, args: string[]): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        child.on('exit', code => {
            if (code === 0) resolve();
            else reject(new Error(`talos-server ${args[0]} exited with code ${code}`));
        });
    });
}
