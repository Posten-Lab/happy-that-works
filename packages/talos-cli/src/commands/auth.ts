...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('talos auth')} - Authentication management

${chalk.bold('Usage:')}
  talos auth login [--force]    Authenticate with Talos
  talos auth logout             Remove authentication and machine data
  talos auth status             Show authentication status
  talos auth help               Show this help message
...
async function handleAuthLogout(): Promise<void> {
  ...
  const talosDir = configuration.talosHomeDir;
  ...
  console.log(chalk.blue('This will log you out of Talos'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use Talos again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove entire talos directory (as current logout does)
      if (existsSync(talosDir)) {
        rmSync(talosDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "talos auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "talos auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "talos auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.talosHomeDir}`));
