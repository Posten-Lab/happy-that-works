...
export interface SpawnSessionOptions {
    ...
    agent?: 'claude' | 'codex' | 'gemini' | 'openclaw' | 'muse';
    environmentVariables?: Record<string, string>;
    token?: string;
    /**
     * If set, the daemon spawns the agent with `--resume <id>` so the new
     * Talos session continues from an existing Claude conversation file.
     * Used by the session fork / duplicate flow: the fork RPC produces a
     * new Claude JSONL on disk, the spawn RPC then attaches a fresh Talos
     * session to it.
     */
    resumeClaudeSessionId?: string;
    /**
     * If set, the daemon spawns Codex with `--resume <id>` so a fresh Talos
