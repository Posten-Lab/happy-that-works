...
describe('parseResumeCommandArgs', () => {
    it('parses the talos session id', () => {
    ...
    it('rejects missing session ids', () => {
        expect(() => parseResumeCommandArgs([])).toThrow(
            'Talos session ID is required: talos resume <session-id>',
        );
    });
});
...
describe('buildResumeLaunch', () => {
    it('builds a Codex resume command', () => {
        expect(buildResumeLaunch({
            ...
            metadata: {
                ...
                host: 'localhost',
                homeDir: '/tmp',
                talosHomeDir: '/tmp/.talos',
                talosLibDir: '/tmp/talos',
                talosToolsDir: '/tmp/talos/tools',
            },
        })).toEqual({
    ...
    it('rejects unsupported flavors', () => {
        ...
        })).toThrow('Talos session session-3 uses unsupported flavor "gemini".');
    });
});

describe('formatResumeHelp', () => {
    it('mentions the session id command shape', () => {
        expect(formatResumeHelp()).toContain('talos resume <talos-session-id>');
    });
});
