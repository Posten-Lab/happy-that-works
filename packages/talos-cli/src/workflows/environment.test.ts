...
import { workflowEnvironment, claudeWorkflowEnvironment } from './environment';
...
it('retains configured Claude authentication and backend selection but not parent routing', () => {
    const env = claudeWorkflowEnvironment({ CLAUDE_CODE_OAUTH_TOKEN: 'test-credential', CLAUDE_CODE_USE_VERTEX: '1', CLAUDE_CONFIG_DIR: '/test/config', CLAUDE_SESSION_ID: 'parent', TALOS_SESSION_ID: 'parent', CODEX_HOME: '/parent' });
    expect(env).toEqual({ CLAUDE_CODE_OAUTH_TOKEN: 'test-credential', CLAUDE_CODE_USE_VERTEX: '1', CLAUDE_CONFIG_DIR: '/test/config' });
});
