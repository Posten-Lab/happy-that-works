...
function detectPosix(): CLIAvailability {
  ...
  return { claude, codex, gemini, openclaw, muse: commandExists('muse') || existsSync(join(os.homedir(), '.local', 'bin', 'muse')), detectedAt: Date.now() };
}
...
function detectWindows(): CLIAvailability {
  ...
  return { claude, codex, gemini, openclaw, muse: checkCommand('muse'), detectedAt: Date.now() };
}
