 ...
 * for talos CLI including configuration, daemon status, logs, and links
 */
...
import { findAllTalosProcesses } from '@/daemon/doctor'
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        ...
        TALOS_PROJECT_ROOT: process.env.TALOS_PROJECT_ROOT,
        DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING: process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING,
        NODE_ENV: process.env.NODE_ENV,
        DEBUG: process.env.DEBUG,
        workingDirectory: process.cwd(),
        processArgv: process.argv,
        talosDir: configuration?.talosHomeDir,
...
/**
 * Slim daemon status output for `talos daemon status`
 */
export async function runDoctorDaemon(): Promise<void> {
    ...
    console.log(chalk.gray('\nRun `talos doctor` for full diagnostics.\n'));
}
...
export async function runDoctorCommand(): Promise<void> {
    ...
    // All Talos processes
    try {
        const allProcesses = await findAllTalosProcesses();
        if (allProcesses.length > 0) {
            console.log(chalk.bold('🔍 All Talos CLI Processes'));
            ...
            if (allProcesses.length > 1) {
                console.log(chalk.bold('\n💡 Process Management'));
                console.log(chalk.gray('To clean up runaway processes: talos doctor clean'));
            }
        } else {
            console.log(chalk.red('❌ No talos processes found'));
        }
    } catch (error) {
    ...
    const wrapperPath = join(projectRoot, 'bin', 'talos.mjs');
    ...
    console.log(`TALOS_HOME_DIR: ${env.TALOS_HOME_DIR ? chalk.green(env.TALOS_HOME_DIR) : chalk.gray('not set')}`);
    console.log(`TALOS_SERVER_URL: ${env.TALOS_SERVER_URL ? chalk.green(env.TALOS_SERVER_URL) : chalk.gray('not set')}`);
    ...
    console.log(chalk.bold('\n🐛 Support & Bug Reports'));
    if (process.env.TALOS_SUPPORT_URL) console.log(`Support: ${chalk.blue(process.env.TALOS_SUPPORT_URL)}`);
    console.log('Command reference: talos --help');

    // ── Concise useful info last (visible without scrolling) ──

    // Basic info
    console.log(chalk.bold('\n📋 Basic Information'));
    console.log(`Talos CLI Version: ${chalk.green(packageJson.version)}`);
    console.log(`Platform: ${chalk.green(process.platform)} ${process.arch}`);
    console.log(`Node.js Version: ${chalk.green(process.version)}`);

    // Configuration
    console.log(chalk.bold('\n⚙️  Configuration'));
    console.log(`Talos Home: ${chalk.blue(configuration.talosHomeDir)}`);
