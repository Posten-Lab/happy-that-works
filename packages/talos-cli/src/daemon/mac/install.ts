/**
 * Installation script for Talos daemon using macOS LaunchDaemons
 *
 * NOTE: This installation method is currently NOT USED in favor of auto-starting
 * the daemon when the user runs the talos command.
 *
 ...
 * 2. We assume users will run talos frequently (every time they open their laptop)
...
const PLIST_LABEL = 'com.talos-cli.daemon';
...
export async function install(): Promise<void> {
    try {
        ...
        // Get the path to the talos CLI executable
        const talosPath = process.argv[0]; // Node.js executable
        ...
        const plistContent = trimIdent(`
            ...
            <dict>
                ...
                <array>
                    <string>${talosPath}</string>
                    <string>${scriptPath}</string>
                    <string>talos-daemon</string>
                </array>

                <key>EnvironmentVariables</key>
                <dict>
                    <key>TALOS_DAEMON_MODE</key>
                    <string>true</string>
                </dict>
                ...
                <string>${os.homedir()}/.talos/daemon.err</string>

                <key>StandardOutPath</key>
                <string>${os.homedir()}/.talos/daemon.log</string>
        ...
        logger.info('Check logs at ~/.talos/daemon.log');
