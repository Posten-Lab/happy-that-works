...
/**
 * Find all Talos CLI processes (including current process)
 */
export async function findAllTalosProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      // Cleanup must identify the CLI, not just a generic JavaScript entrypoint.
...
/**
 * Find all runaway Talos CLI processes that should be killed
 */
export async function findRunawayTalosProcesses(): Promise<Array<{ pid: number, command: string }>> {
...
/**
 * Kill all runaway Talos CLI processes
 */
export async function killRunawayTalosProcesses(): Promise<{ killed: number, errors: Array<{ pid: number, error: string }> }> {
  const runawayProcesses = await findRunawayTalosProcesses();
