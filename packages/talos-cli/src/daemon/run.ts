...
// Suffix host with `-dev` for the TALOS_VARIANT=dev variant so the dev daemon
// is visually distinct from the stable one in the machine list (they otherwise
// share the same hostname and look identical).
const hostSuffix = process.env.TALOS_VARIANT === 'dev' ? '-dev' : '';
  ...
  try {
    ...
    for (const [id, s] of Object.entries(persisted)) {
      sessionIdToFinishedSession.set(id, {
        startedBy: 'persisted',
        talosSessionId: id,
        talosSessionMetadataFromLocalWebhook: s.metadata,
        encryption: {
    ...
    const onTalosSessionWebhook = (sessionId: string, sessionMetadata: Metadata, encryption?: SessionEncryptionData) => {
      ...
      if (existingSession && existingSession.startedBy === 'daemon') {
        ...
        const trackedSession: TrackedSession = {
          startedBy: 'talos directly - likely by user from terminal',
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (options.parentSessionId) {
          extraEnv.TALOS_FORKED_FROM_SESSION_ID = options.parentSessionId;
        }
        if (options.forkedFromMessageId) {
          extraEnv.TALOS_FORKED_FROM_MESSAGE_ID = options.forkedFromMessageId;
        }
        // For fork: spawned Talos CLI needs to know which Claude JSONL to
        // backfill into the fresh Talos session row. Without this, the
        ...
        if (options.resumeClaudeSessionId) {
          extraEnv.TALOS_FORK_CLAUDE_SESSION_ID = options.resumeClaudeSessionId;
        }
        if (options.resumeCodexThreadId) {
          extraEnv.TALOS_FORK_CODEX_THREAD_ID = options.resumeCodexThreadId;
        }
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          const agent = options.agent === 'muse' ? 'muse' : options.agent === 'gemini' ? 'gemini' : (options.agent === 'codex' ? 'codex' : (options.agent === 'openclaw' ? 'openclaw' : 'claude'));
          ...
          const resumeFragment = resumeId
            ...
            : '';
          const fullCommand = `node --no-warnings --no-deprecation ${cliPath} ${agent} --talos-starting-mode remote --started-by daemon${resumeFragment}`;
          ...
          const windowName = `talos-${Date.now()}-${agent}`;
          ...
          if (tmuxResult.success) {
            ...
            // Wait for webhook to populate session with talosSessionId (exact same as regular flow)
            ...
            return new Promise((resolve) => {
              ...
              pidToAwaiter.set(tmuxResult.pid!, (completedSession) => {
                ...
                logger.debug(`[DAEMON RUN] Session ${completedSession.talosSessionId} fully spawned with webhook (tmux)`);
                resolve({
                  type: 'success',
                  sessionId: completedSession.talosSessionId!
                });
              });
            });
          } else {
        ...
        if (!useTmux) {
          ...
          const args = [
            agentCommand,
            '--talos-starting-mode', 'remote',
            '--started-by', 'daemon'
          ];

          // Resume ids attach the new Talos session to a pre-existing provider
    ...
    const spawnTrackedTalosProcess = ({
      args,
      cwd,
      env,
      ...
      if (!talosProcess.pid) {
        ...
        return Promise.resolve({
          type: 'error',
          errorMessage: 'Failed to spawn Talos process - no PID returned'
        });
      }

      logger.debug(`[DAEMON RUN] Spawned process with PID ${talosProcess.pid}`);
      ...
      pidToTrackedSession.set(talosProcess.pid, trackedSession);
      ...
      logger.debug(`[DAEMON RUN] Waiting for session webhook for PID ${talosProcess.pid}`);
    ...
    const findTrackedSessionById = (talosSessionId: string): TrackedSession | undefined => {
      for (const session of pidToTrackedSession.values()) {
        if (session.talosSessionId === talosSessionId) return session;
      }
    ...
    const onChildExited = (pid: number) => {
      ...
      if (session?.talosSessionId && session.encryption) {
        sessionIdToFinishedSession.set(session.talosSessionId, session);
        logger.debug(`[DAEMON RUN] Process PID ${pid} exited, preserved session ${session.talosSessionId} for resume`);
      } else {
    ...
    const { port: controlPort, stop: stopControlServer } = await startDaemonControlServer({
      ...
      stopSession,
      spawnSession,
      requestShutdown: () => requestShutdown('talos-cli'),
      onTalosSessionWebhook
    });
    ...
    // 4. Write heartbeat
    const heartbeatIntervalMs = parseInt(process.env.TALOS_DAEMON_HEARTBEAT_INTERVAL || '60000');
    ...
    const restartOnStaleVersionAndHeartbeat = setInterval(async () => {
      ...
      if (bundleReplaced) {
        ...
        // isDaemonRunningCurrentlyInstalledTalosVersion() === true, and exits —
