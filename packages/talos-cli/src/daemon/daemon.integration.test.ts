 ...
 *
 * This file boots one authenticated Talos environment and restarts the daemon
...
async function stopAllTrackedSessions(): Promise<void> {
  ...
  await Promise.all(
    sessions.map((session: any) =>
      stopDaemonSession(session.talosSessionId ?? `PID-${session.pid}`).catch(() => false)
    ),
  );
}
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnTalosCLI(['daemon', 'start'], {
      stdio: 'ignore',
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      talosHomeDir: '/test/talos-home',
      talosLibDir: '/test/talos-lib',
      talosToolsDir: '/test/talos-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('talos directly - likely by user from terminal');
    expect(tracked.talosSessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.talosSessionId === response.sessionId
    );
    ...
    expect(spawnedSession.talosSessionId).toBeDefined();
    await stopDaemonSession(spawnedSession.talosSessionId);
  });
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    // Spawn a real talos process that looks like it was started from terminal
    const terminalTalosProcess = spawnTalosCLI([
    ...
    if (!terminalTalosProcess || !terminalTalosProcess.pid) {
      throw new Error('Failed to spawn terminal talos process');
    }
    ...
    const terminalSession = sessions.find(
      (s: any) => s.pid === terminalTalosProcess.pid
    );
    const daemonSession = sessions.find(
      (s: any) => s.talosSessionId === spawnResponse.sessionId
    );

    expect(terminalSession).toBeDefined();
    expect(terminalSession.startedBy).toBe('talos directly - likely by user from terminal');
    ...
    await stopDaemonSession(daemonSession.talosSessionId);
    
    // Also kill the terminal process directly to be sure
    try {
      terminalTalosProcess.kill('SIGTERM');
    } catch (e) {
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.talosSessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.talosSessionId)
    );
    expect(daemonSessions.length).toBeGreaterThanOrEqual(3);

    // Stop all spawned sessions
    for (const session of daemonSessions) {
      expect(session.talosSessionId).toBeDefined();
      await stopDaemonSession(session.talosSessionId);
    }
  });
   ...
   * 6. Daemon spawns new daemon via spawnTalosCLI(['daemon', 'start'])
   ...
   * 
   * This simulates what happens during `npm upgrade talos`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/talos/ with NEW version files
  ...
  it.skip('[skipped] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.TALOS_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when talos is uninstalled
