...
export function startDaemonControlServer({
  ...
  onTalosSessionWebhook: (sessionId: string, metadata: Metadata, encryption?: SessionEncryptionData) => void;
  ...
  return new Promise((resolve) => {
    ...
    typed.post('/session-started', {
      ...
      onTalosSessionWebhook(sessionId, metadata, encryptionData);
    ...
    typed.post('/list', {
      ...
      return { 
        children: children
          .filter(child => child.talosSessionId !== undefined)
          .map(child => ({
            startedBy: child.startedBy,
            talosSessionId: child.talosSessionId!,
            pid: child.pid
          }))
      }
    });
    ...
    typed.post('/spawn-session', {
      schema: {
        body: z.object({
          ...
          agent: z.enum(['claude', 'codex', 'gemini', 'openclaw', 'muse']).optional(),
