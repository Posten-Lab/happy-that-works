# Talos CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages Talos sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `talos daemon start`

Control Flow:
1. `src/index.ts` receives `daemon start` command
2. Spawns detached process via `spawnTalosCLI(['daemon', 'start-sync'], { detached: true })`
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-talos-session`, `stop-session`, `requestShutdown` handlers
   - Heartbeat loop: every 60s (or TALOS_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
The daemon detects when `npm upgrade talos` occurs:
...
3. If mismatch detected:
   - Spawns new daemon via `spawnTalosCLI(['daemon', 'start'])`
...
1. Backend forwards RPC `spawn-talos-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached Talos process with `--talos-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New Talos process:
   - Creates session with backend, receives `talosSessionId`
   - Calls `notifyDaemonSessionStarted()` to POST to daemon's `/session-started`
5. Daemon updates tracking with `talosSessionId`, resolves awaiter
6. RPC returns session info to mobile app

### Terminal-Spawned Sessions

User runs `talos` directly:
1. CLI auto-starts daemon if configured
2. Talos process calls `notifyDaemonSessionStarted()`
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'talos directly...'`
4. Session tracked for health monitoring

### Session Termination

Via RPC `stop-session` or health check:
1. `stopSession()` finds session by `talosSessionId`
...
`talos doctor` uses `ps aux | grep` to find all Talos processes:
- Production: matches `talos.mjs`, `talos`, `dist/index.mjs`
...
`talos doctor clean`:
1. `findRunawayTalosProcesses()` filters for likely orphans
2. `killRunawayTalosProcesses()`:
   - Sends SIGTERM
   - Waits 1 second
...
- Server to Daemon: rpc-request (spawn-talos-session, stop-session, requestShutdown)
...
```json
{
  ...
  "metadata": "base64(encrypted({
    ...
    'platform': 'darwin',
    'talosCliVersion': '1.0.0',
    'homeDir': '/Users/john',
    'talosHomeDir': '/Users/john/.talos'
  }))",
...
socket.emit('machine-update-metadata', {
  ...
  "metadata": "base64(encrypted({
    'host': 'MacBook-Pro.local',
    'platform': 'darwin',
    'talosCliVersion': '1.0.1',  // version updated
    'homeDir': '/Users/john',
    'talosHomeDir': '/Users/john/.talos'
  }))",
  "expectedVersion": 1
