...
const testMetadata = {
    ...
    talosToolsDir: '/home/user/.talos/tools'
};
...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when Talos server is unreachable (ECONNREFUSED)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('⚠️  Talos server unreachable')
            );

            consoleSpy.mockRestore();
        });

        it('should return null when Talos server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when Talos server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
