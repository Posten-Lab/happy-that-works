...
import { detectResumeSupport, type ResumeSupport } from '@/resume/localTalosAgentAuth';
...
export class ApiMachineClient {
    ...
    setRPCHandlers({
        ...
        // Claude UUID. The caller then spawns a fresh Talos session with
        ...
        // composer can discover models before a Talos session exists.
    ...
    private syncResumeSessionRpcRegistration(): void {
        const method = rpcMethods.resumeSession;
    ...
    async updateMachineMetadata(handler: (metadata: MachineMetadata | null) => MachineMetadata): Promise<void> {
        await backoff(async () => {
            ...
            const answer = await this.socket.emitWithAck('machine-update-metadata', {
                ...
                metadata: encodeBase64(encrypt(this.machine.encryptionKey, this.machine.encryptionVariant, toWireMetadata(updated))),
                expectedVersion: this.machine.metadataVersion
            });

            if (answer.result === 'success') {
                this.machine.metadata = normalizeMetadata(decrypt(this.machine.encryptionKey, this.machine.encryptionVariant, decodeBase64(answer.metadata)));
    ...
    connect() {
        ...
        this.socket = io(serverUrl, {
            ...
            auth: {
                ...
                machineId: this.machine.id,
                talosClient: `cli-daemon/${configuration.currentCliVersion}`
            },
            path: '/v1/updates',
            reconnection: false,
        });
        ...
        this.socket.on('update', (data: Update) => {
            ...
            if (data.body.t === 'update-machine' && (data.body as UpdateMachineBody).machineId === this.machine.id) {
                ...
                if (update.metadata) {
                    ...
                    this.machine.metadata = normalizeMetadata(decrypt(this.machine.encryptionKey, this.machine.encryptionVariant, decodeBase64(update.metadata.value)));
