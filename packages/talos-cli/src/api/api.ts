  ...
  async getOrCreateSession(opts: {
    ...
    try {
      const response = await axios.post<CreateSessionResponse>(
        ...
        {
          tag: opts.tag,
          metadata: encodeBase64(encrypt(encryptionKey, encryptionVariant, toWireMetadata(opts.metadata))),
      ...
      let session: Session = {
        id: raw.id,
        seq: raw.seq,
        metadata: normalizeMetadata(decrypt(encryptionKey, encryptionVariant, decodeBase64(raw.metadata))),
  ...
  async getOrCreateMachine(opts: {
    ...
    try {
      ...
      const machine: Machine = {
        ...
        encryptionKey: encryptionKey,
        encryptionVariant: encryptionVariant,
        metadata: raw.metadata ? normalizeMetadata(decrypt(encryptionKey, encryptionVariant, decodeBase64(raw.metadata))) : null,
      ...
      if (axios.isAxiosError(error) && error.response?.status) {
        ...
        if (status === 403 || status === 409) {
          ...
          console.log(chalk.yellow(
            `   → Run 'talos doctor clean' to reset local state and generate a new machine ID`
          ));
          console.log(chalk.yellow(
