...
function buildMultipartUploadBody(
    ...
    const boundary = `----talos-cli-${randomUUID()}`;
    const chunks: Buffer[] = [];
...
export class ApiSessionClient extends EventEmitter {
    ...
    constructor(token: string, session: Session) {
        ...
        this.socket = io(configuration.serverUrl, {
            auth: {
                ...
                talosClient: `cli-coding-session/${configuration.currentCliVersion}`
            },
            path: '/v1/updates',
            reconnection: false,
     ...
     * Legacy sessions use deriveKey(masterSecret, encryptionContexts.blobs, ['master']).
     * DataKey sessions use deriveKey(dataKey, encryptionContexts.blobs, ['session']).
     */
    async getBlobKey(): Promise<Uint8Array> {
        if (!this.blobKey) {
            const path = this.encryptionVariant === 'dataKey' ? ['session'] : ['master'];
            this.blobKey = await deriveKey(this.encryptionKey, encryptionContexts.blobs, path);
        }
        return this.blobKey;
    }
