...
it('accepts session resume requests from an earlier client', async () => {
    const resumeSession = vi.fn().mockResolvedValue({ type: 'success', sessionId: 'existing-session' });
    const { ApiMachineClient } = await import('./apiMachine');
    const client = new ApiMachineClient('token', machineClient());
    client.setRPCHandlers({ spawnSession: vi.fn(), stopSession: vi.fn(), requestShutdown: vi.fn(), resumeSession });

    const handler = handlersFrom(client).get('machine-1:resume-happy-session');
    expect(handler).toBeDefined();
    await expect(handler!({ sessionId: 'existing-session', model: 'selected-model', permissionMode: 'default' }))
        .resolves.toEqual({ type: 'success', sessionId: 'existing-session' });
    expect(resumeSession).toHaveBeenCalledWith('existing-session', { model: 'selected-model', permissionMode: 'default' });
});
...
describe('ApiMachineClient Codex fork RPCs', () => {
    ...
    it('forwards resumeCodexThreadId through the spawn RPC', async () => {
        const spawnSession = vi.fn().mockResolvedValue({ type: 'success', sessionId: 'talos-forked' });
