 ...
 * through the plan, which Talos folds into a pinned todo list — so Codex must
 * keep its plan tool current rather than using it at its own discretion.
 */
const TALOS_DEVELOPER_INSTRUCTIONS = [
...
// Codex 0.152.0 made update_plan opt-in. Talos renders its notifications as the
...
export class CodexAppServerClient {
    ...
    private handleNotification(method: string, params: any): void {
        ...
        // (e.g. talos-mcp bridge failing on Windows due to shebang execution).
