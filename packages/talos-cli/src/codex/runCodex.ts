...
export async function runCodex(opts: {
    ...
    try {
        ...
        console.error('  \x1b[36mtalos claude\x1b[0m\n');
        process.exit(1);
    }
    ...
    const handleUserMessage = createSerialAsyncHandler<UserMessage>(async (message) => {
        ...
        // provider rollout becomes selectable without restarting Talos.
        try {
    ...
    // Start Talos MCP server (HTTP) and prepare STDIO bridge config for Codex
    ...
    const bridgeEntrypoint = join(projectPath(), 'bin', 'talos-mcp.mjs');
    const mcpServers = {
        talos: {
            command: process.execPath,
            args: ['--no-warnings', '--no-deprecation', bridgeEntrypoint, '--url', talosServer.url]
        }
    } as const;
    let first = true;
    ...
    try {
        ...
        const forkCodexThreadId = process.env.TALOS_FORK_CODEX_THREAD_ID;
        ...
        // Stop Talos MCP server
        logger.debug('[codex]: talosServer.stop');
        talosServer.stop();
