 ...
 * The daemon spawns this as: `node dist/index.mjs openclaw --talos-starting-mode remote --started-by daemon`
 *
 * Connects to an OpenClaw gateway via WebSocket, translates the gateway protocol
 * to Talos's AgentMessage format, and forwards everything through the session pipeline.
 */
...
export async function runOpenClaw(opts: RunOpenClawOptions): Promise<void> {
  ...
  if (response) {
    log(`Talos Session ID: ${response.id}`);
  }
