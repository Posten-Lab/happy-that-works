...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    mockProjectPath: vi.fn(() => '/tmp/talos'),
...
vi.mock('@/daemon/run', () => ({
  initialMachineMetadata: { host: 'host', platform: 'darwin', talosCliVersion: 'test', homeDir: '/tmp', talosHomeDir: '/tmp/.talos', talosLibDir: '/tmp/talos' },
}));
...
vi.mock('@/claude/utils/startTalosServer', () => ({
  startTalosServer: mocks.mockStartTalosServer,
}));
...
describe('runAcp', () => {
  ...
  beforeEach(() => {
    ...
    mocks.mockStartTalosServer.mockResolvedValue({
  ...
  it('updates session metadata with ACP config options (models and operating modes)', async () => {
    ...
    const baseMetadata = {
      ...
      talosLibDir: '/repo/.talos/lib',
      talosToolsDir: '/repo/.talos/tools',
    };
