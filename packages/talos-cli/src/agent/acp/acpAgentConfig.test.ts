...
describe('resolveAcpAgentConfig', () => {
  ...
  it('throws with helpful usage when no args are provided', () => {
    expect(() => resolveAcpAgentConfig([])).toThrow('Usage: talos acp <agent-name> or talos acp -- <command> [args]');
  });

  it('throws when separator form omits command', () => {
    expect(() => resolveAcpAgentConfig(['--'])).toThrow('Missing command after "--". Usage: talos acp -- <command> [args]');
  });
});
