/**
 * Minimal persistence functions for talos CLI
 * 
 * Handles settings and private key storage in ~/.talos/ or local .talos/
 */
...
interface Settings {
  ...
  daemonAutoStartWhenRunningTalos?: boolean
  chromeMode?: boolean
  ...
  try {
    ...
    if (schemaVersion > SUPPORTED_SCHEMA_VERSION) {
      logger.warn(
        ...
        'Update talos-cli for full functionality.'
      );
    }
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.talosHomeDir)) {
    await mkdir(configuration.talosHomeDir, { recursive: true })
  }
