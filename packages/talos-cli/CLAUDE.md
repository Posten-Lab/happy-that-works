# Talos CLI Codebase Overview

## Project Overview

Talos CLI (`handy-cli`) is a command-line tool that wraps Claude Code to enable remote control and session sharing. It's part of a three-component system:

1. **handy-cli** (this project) - CLI wrapper for Claude Code
2. **handy** - React Native mobile client
3. **handy-server** - Node.js server with Prisma (hosted at https://api.talos-servers.com/)
...
its renderer off tool name (see `talos-app/sources/sync/reducer/taskTools.ts`).
...
and check that `talos-app`'s `knownTools` registry covers whatever is emitted.
...
## Starting the Daemon
```bash
# From the talos-cli directory:
./bin/talos.mjs daemon start

# With custom server URL (for local development):
TALOS_SERVER_URL=http://localhost:3005 ./bin/talos.mjs daemon start

# Stop the daemon:
./bin/talos.mjs daemon stop

# Check daemon status:
./bin/talos.mjs daemon status
```

## Daemon Logs
- Daemon logs are stored in `~/.talos-dev/logs/` (or `$TALOS_HOME_DIR/logs/`)
