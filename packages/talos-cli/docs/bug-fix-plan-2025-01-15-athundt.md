# Minimal Fix Plan for Talos-CLI Bugs with TDD
# Date: 2025-01-15
...
**Problem**: When running `./bin/talos.mjs --continue`, Claude CLI returns error:
```
Error: --session-id cannot be used with --continue or --resume
```

**Root Cause Analysis**:
- This is a Claude Code 2.0.64+ design constraint, NOT a talos-cli bug
- Talos-CLI generates a NEW session ID and adds `--session-id <uuid>` for all local sessions
...
2. **Remote Mode (No conflict)**:
   ...
   → No --session-id added by talos-cli
   → Works fine
   ```
...
## Bug 2: Talos Server Unavailability Crash

**Problem**: Talos-CLI crashes when Talos API server is unreachable

**Server Details**:
- Default server: `https://api.cluster-fluster.com`
- Environment variable: `TALOS_SERVER_URL` (overrides default)
- Local development: `http://localhost:3005`
- The server handles session management and real-time communication for Talos CLI
...
try {
    ...
    console.log('⚠️  Cannot connect to Talos server - continuing in local mode');
...
```typescript
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        console.log('⚠️  Talos server unreachable - working in offline mode');
...
describe('ApiSessionClient connection handling', () => {
    it('should handle socket connection failure gracefully', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            '⚠️  Cannot connect to Talos server - continuing in local mode'
        );
...
describe('Api server error handling', () => {
    it('should return null when Talos server is unreachable', async () => {
        const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            '⚠️  Talos server unreachable - working in offline mode'
        );
...
5. **Refactor Phase**:
   - Add session ID extraction for --continue (future enhancement):
     ...
     - Update Talos's session metadata with Claude's ID
   - Ensure code is clean and minimal

6. **Manual Verification**:
   ```bash
   # Test Bug 1 fix:
   ./bin/talos.mjs --continue  # Should work without error
   # Verify mobile/daemon still work with session ID

   # Test Bug 2 fix:
   TALOS_SERVER_URL=http://invalid:9999 ./bin/talos.mjs  # Should show warning, not crash
...
**Bug 1 Fixed**:
- Test: `./bin/talos.mjs --continue` exits with code 0
- No "session-id cannot be used" error

**Bug 2 Fixed**:
- Test: `TALOS_SERVER_URL=http://invalid:9999 ./bin/talos.mjs` shows warning message
- Process continues in local mode instead of crashing
- Clear user feedback: "⚠️ Talos server unreachable - working in offline mode"
