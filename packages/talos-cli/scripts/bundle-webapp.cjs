...
/**
 * Runs `expo export -p web` in packages/talos-app and copies the output into
 * a package-owned artifact directory. By default this writes to talos-cli/tools/webapp
 * for local development. Release packaging can pass --out-dir to place it elsewhere.
 *
 * talos-cli does NOT depend on talos-app — we reach into the sibling at build time only.
 */
...
const APP_DIR = path.resolve(REPO_ROOT, 'packages/talos-app');
...
function main() {
    ...
    console.log(`→ Building talos-app web bundle (expo export)`);
    rmrf(APP_DIST);
