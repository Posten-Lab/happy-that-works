...
/**
 * Bundles talos-server into a self-contained artifact directory.
 *
 * Uses `bun build --compile` to produce a single platform-specific binary, then copies the
 * pglite WASM/data files and prisma migrations alongside. talos-cli does NOT depend on the
 * talos-server workspace package — we reach into the sibling directory at build time only.
 *
 * Layout produced:
 *   <out-dir>/
 *     <platform>/
 *       talos-server                  # bun-compiled binary
...
const SERVER_DIR = path.resolve(REPO_ROOT, 'packages/talos-server');
...
function platformBinaryName(plat) {
    return plat.endsWith('-win32') ? 'talos-server.exe' : 'talos-server';
}
...
function buildPlatform(plat, outRoot) {
    ...
    console.log(`\n→ Bundling talos-server for ${plat} (${target})`);
    run(
        'bun',
        [
            'build',
...
function main() {
    ...
    console.log(`\n✓ talos-server bundle written to ${outDir}`);
}
