...
for cmd in kubectl node minikube curl; do
...
if $DEPLOY; then
    ...
    bash "$SCRIPT_DIR/local.sh" 2>&1
fi

# --- Wait for pods ---
echo ""
echo -e "${BOLD}Waiting for pods...${NC}"
kubectl --namespace talos-integration wait --for=condition=ready pod -l app=talos-server --timeout=120s
PODS=$(kubectl --namespace talos-integration get pods -l app=talos-server --no-headers | wc -l | tr -d ' ')
echo -e "${GREEN}$PODS server pods ready${NC}"

# --- Server URL: use minikube tunnel for NodePort (real LB, survives pod kills) ---
setup_server_url() {
    local svc_type
    svc_type=$(kubectl --namespace talos-integration get svc talos-server -o jsonpath='{.spec.type}' 2>/dev/null)
    ...
    if [ "$svc_type" = "NodePort" ]; then
        ...
        local tmpfile
        tmpfile=$(mktemp)
        minikube service --namespace talos-integration talos-server --url > "$tmpfile" 2>/dev/null &
        PF_PIDS+=($!)
        sleep 2
        ...
        kubectl --namespace talos-integration port-forward svc/talos-server 3005:3000 &>/dev/null &
        PF_PIDS+=($!)
    ...
    kubectl --namespace talos-integration port-forward svc/talos-redis "$REDIS_PORT:6379" &>/dev/null &
    PF_PIDS+=($!)
...
wait_for_pods() {
    ...
    for i in {1..60}; do
        READY=$(kubectl --namespace talos-integration get pods -l app=talos-server --no-headers 2>/dev/null | grep -c "Running" || true)
        if [ "$READY" -ge 2 ]; then
            break
        fi
        sleep 1
    done
    kubectl --namespace talos-integration wait --for=condition=ready pod -l app=talos-server --timeout=120s 2>/dev/null || true
}
