...
function getPods() {
    return execSync("kubectl --namespace talos-integration get pods -l app=talos-server -o jsonpath='{.items[*].metadata.name}'")
        .toString().replace(/'/g, "").trim().split(/\s+/);
}

function findSocketPod(socketId, pods) {
    for (const pod of pods) {
        const logs = execSync(`kubectl --namespace talos-integration logs ${pod} --tail=500 2>/dev/null`).toString();
...
function waitForPodReady(label, timeoutSec = 60) {
    ...
    while (Date.now() < deadline) {
        try {
            const out = execSync(
                `kubectl --namespace talos-integration get pods -l ${label} -o jsonpath='{.items[*].status.conditions[?(@.type=="Ready")].status}'`
...
async function testPodKill(token) {
    ...
    try {
        ...
        try {
            execSync(`kubectl --namespace talos-integration delete pod ${daemonPod} --force --grace-period=0 2>&1`);
        } catch (e) {
            console.log(`  kubectl --namespace talos-integration delete output: ${e.stdout?.toString() || e.message}`);
        }
        ...
        waitForPodReady("app=talos-server", 90);
