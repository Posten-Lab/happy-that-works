...
function kc(args) {
    return execSync(`kubectl --namespace talos-integration ${args}`).toString().trim();
}

function getPods() {
    return kc("get pods -l app=talos-server -o jsonpath='{.items[*].metadata.name}'")
        .replace(/'/g, "")
        .split(/\s+/)
        .filter(Boolean);
}
...
function findPodFor(socketId) {
    for (const p of getPods()) {
        try {
            const out = execSync(`kubectl --namespace talos-integration logs ${p} --tail=2000 2>/dev/null`).toString();
...
async function rollingDeploy() {
    ...
    if (immediateOk < NUM_DAEMONS * 0.5) {
        ...
        daemons.forEach(d => d.socket.disconnect());
        callers.forEach(c => c.disconnect());
        try { kc("wait --for=condition=ready pod -l app=talos-server --timeout=120s"); } catch {}
        return false;
    }
...
// 1. Machine daemon connects + registers spawn-talos-session (fire-and-forget)
// 2. "iOS" caller immediately calls spawn-talos-session via RPC
...
async function iosSessionFlow() {
    ...
    for (let round = 0; round < ROUNDS; round++) {
        ...
        const MACHINE_METHOD = `${machineId}:spawn-talos-session`;
...
async function crossReplica3Pod() {
    ...
    log(`Pods: ${pods.join(", ")} (${pods.length} replicas)`);
    if (pods.length < 3) {
        log("⚠️  Need 3+ replicas — skipping (scale up: kubectl --namespace talos-integration scale deployment/talos-server --replicas=3)");
        return true;
    }
