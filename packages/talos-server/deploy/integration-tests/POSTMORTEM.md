...
## Setup
- 2 talos-server replicas behind a `LoadBalancer` service via `minikube tunnel`
...
**Observation:** `kubectl exec talos-redis-0 -- redis-cli XINFO STREAM socket.io`
...
1. **Re-register on every reconnect** (client side, daemon)
    - Already done by talos-cli for fresh connects but the
