...
`pnpm --filter talos-server build` needs Bun on PATH to bundle the Node.js runtime. `pnpm --filter talos-server bundle:webapp` adds the Expo web export to this package. `pnpm verify:packaged` checks the actual installed tarballs, database, and bundled web page.
...
The repository's `Dockerfile` builds a standalone relay image; `Dockerfile.server` builds the service used with external storage. See [production deployment templates](../../deploy/server) and the [Talos release requirements](../../docs/rebrand/README.md) for operator configuration. Packages remain private until the Talos publishing namespace is confirmed.
