...
export async function runMigrations(opts: { pgliteDir: string; migrationsDir?: string } = { pgliteDir }) {
    ...
    candidates.push(
        ...
        path.join(process.cwd(), "packages", "talos-server", "prisma", "migrations"),
...
async function serve() {
    ...
    if (process.env.TALOS_INJECT_HTML_CONFIG) {
        try {
            injectHtmlConfig = JSON.parse(process.env.TALOS_INJECT_HTML_CONFIG);
        } catch {
...
export function isStandaloneEntrypoint(invokedFile: string): boolean {
    ...
    // talos-server.exe when tests or tooling run cross-platform.
...
if (isDirectInvocation) {
    ...
    switch (command) {
        ...
        case "serve":
            ...
            break;
        default:
            console.log(`talos-server - portable distribution
...
Environment variables:
  ...
  TALOS_MASTER_SECRET  Required: master secret for auth/encryption
`);
