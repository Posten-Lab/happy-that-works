...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect(`${process.env.TALOS_WEBAPP_URL || 'http://localhost:8081'}?error=invalid_state`);
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect(`${process.env.TALOS_WEBAPP_URL || 'http://localhost:8081'}?error=server_config`);
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(`${process.env.TALOS_WEBAPP_URL || 'http://localhost:8081'}?error=${encodeURIComponent(tokenResponseData.error)}`);
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect(`${process.env.TALOS_WEBAPP_URL || 'http://localhost:8081'}?error=github_user_fetch_failed`);
            }
            ...
            return reply.redirect(`${process.env.TALOS_WEBAPP_URL || 'http://localhost:8081'}?github=connected&user=${encodeURIComponent(userData.login)}`);

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect(`${process.env.TALOS_WEBAPP_URL || 'http://localhost:8081'}?error=server_error`);
        }
    });
