...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, TALOS_HOME_DIR: '/tmp/nonexistent-talos-test' } });
...
describe('talos-agent CLI', () => {
    it('should display help output', () => {
        const { stdout } = runCli('--help');
        expect(stdout).toContain('talos-agent');
        expect(stdout).toContain('CLI client for controlling Talos agents remotely');
    });
