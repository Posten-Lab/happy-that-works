...
function decryptSession(raw: RawSession, creds: Credentials): DecryptedSession {
    ...
    return {
        ...
        metadata: normalizeMetadata(decryptField(raw.metadata, encryption)),
...
function handleApiError(err: unknown, context: string): never {
    if (err instanceof AxiosError) {
        ...
        if (status === 401) {
            throw new Error('Authentication expired. Run `talos-agent auth login` to re-authenticate.');
        }
        if (status === 403) {
...
export async function createSession(
    ...
    const encryptedMetadata = encryptWithDataKey(toWireMetadata(opts.metadata), sessionKey);
