...
function ensureMachineCanResume(machine: DecryptedMachine): void {
    const metadata = (machine.metadata ?? {}) as {
        resumeSupport?: {
            ...
            talosAgentAuthenticated?: unknown;
        };
    };
    ...
    if (metadata.resumeSupport?.talosAgentAuthenticated === false) {
        throw new Error('Resume is unavailable on this machine. Run `talos-agent auth login` in that machine environment first.');
    }
...
program
    .name('talos-agent')
    .description('CLI client for controlling Talos agents remotely')
    .version('0.1.0');
