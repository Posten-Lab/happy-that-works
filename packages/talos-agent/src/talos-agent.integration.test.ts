...
const binPath = resolve(packageDir, 'bin', 'talos-agent.mjs');
const keepIntegrationEnv = ['1', 'true', 'yes'].includes((process.env.TALOS_AGENT_KEEP_ENV ?? '').toLowerCase());
...
function agentEnvVars(serverPort: number, homeDir: string): NodeJS.ProcessEnv {
    return {
        ...process.env,
        TALOS_SERVER_URL: `http://localhost:${serverPort}`,
        TALOS_HOME_DIR: homeDir,
    };
}
...
function createGitProject(envDir: string): { projectDir: string; worktreeDir: string } {
    const projectDir = join(envDir, 'talos-agent-test-project');
    const worktreeDir = join(projectDir, '.dev', 'worktree', 'feature-branch');

    mkdirSync(projectDir, { recursive: true });
    runCommand('git', ['init', '--initial-branch=main'], projectDir);
    runCommand('git', ['config', 'user.name', 'Talos Agent Test'], projectDir);
    runCommand('git', ['config', 'user.email', 'talos-agent-tests@example.com'], projectDir);

    writeFile(join(projectDir, 'README.md'), '# Talos Agent Test Project\n');
...
async function waitForSessionInList(sessionId: string, env: NodeJS.ProcessEnv): Promise<void> {
    ...
    }, 20_000, `session ${sessionId} to appear in talos-agent list`);
}
...
async function approveAgentLogin(
    ...
    const response = await fetch(`${serverUrl}/v1/auth/account/response`, {
        ...
        headers: {
            ...
            'X-Talos-Client': 'cli-control-plane/0.1.0',
        },
        body: JSON.stringify({
...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for talos-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`talos-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
async function listDaemonSessions(httpPort: number): Promise<Array<{ talosSessionId: string; pid: number; startedBy: string }>> {
    ...
    const parsed = await response.json() as { children: Array<{ talosSessionId: string; pid: number; startedBy: string }> };
...
describe('talos-agent integration', { timeout: 180_000 }, () => {
    beforeAll(async () => {
        ...
        if (keepIntegrationEnv) {
            console.log(`[talos-agent integration] keeping environment: ${integrationEnvName}`);
            console.log(`[talos-agent integration] environment dir: ${integrationEnvDir}`);
        }
    });
    ...
    it('authenticates, lists machines, and spawns a session through the real daemon RPC path', async () => {
        ...
        const machines = JSON.parse(runAgentCli(['machines', '--json'], agentEnv)) as Array<{
            ...
            metadata?: {
                ...
                resumeSupport?: {
                    ...
                    talosAgentAuthenticated?: boolean;
                };
            };
        }>;
        ...
        await waitFor(async () => {
            ...
            return refreshedMachine?.metadata?.resumeSupport?.rpcAvailable === true
                && refreshedMachine.metadata.resumeSupport.talosAgentAuthenticated === true;
        ...
        await waitFor(async () => {
            ...
            return sessions.some(session => session.talosSessionId === sessionId);
        }, 20_000, 'spawned session to be tracked by daemon');
    });

    it('spawns in the test project root and sends a message through talos-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testProjectDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'talos-agent root message';
    ...
    it('spawns in a git worktree and sends a message through talos-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testWorktreeDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'talos-agent worktree message';
        const spawnResult = parseJson<{
            type: 'success' | 'requestToApproveDirectoryCreation' | 'error';
            sessionId?: string;
            machineId?: string;
