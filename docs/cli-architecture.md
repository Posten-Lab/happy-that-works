...
This document describes the Talos CLI (`packages/talos-cli`) and its daemon. The CLI is both an interactive tool and a background session manager that keeps machine state in sync with the server.
...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.talos`.
...
```mermaid
flowchart TD
    Start([talos ...]) --> Parse[Parse subcommand]
...
graph LR
    ...
    subgraph "Environment Overrides"
        ...
        E3[TALOS_WEBAPP_URL]
        E4[TALOS_VARIANT]
        E5[TALOS_EXPERIMENTAL]
        E6[TALOS_DISABLE_CAFFEINATE]
    end

    E1 -.-> settings & access & daemon & logs
```

Local state lives under `~/.talos` (or `TALOS_HOME_DIR`):
...
- `TALOS_SERVER_URL` and `TALOS_WEBAPP_URL` override defaults.
- `TALOS_VARIANT`, `TALOS_EXPERIMENTAL`, `TALOS_DISABLE_CAFFEINATE` control behavior.
...
- CLI entry: `packages/talos-cli/src/index.ts`
- Daemon: `packages/talos-cli/src/daemon`
- Control server/client: `packages/talos-cli/src/daemon/controlServer.ts`, `packages/talos-cli/src/daemon/controlClient.ts`
- API clients: `packages/talos-cli/src/api`
- Persistence: `packages/talos-cli/src/persistence.ts`
- Config: `packages/talos-cli/src/configuration.ts`
