...
Talos session. The manager inspects and steers that session with `talos-agent`.
...
- Source the current project Talos environment before running `talos-agent`.
- Spawn engineer sessions with `talos-agent`.
...
1. Control plane: shared Talos account/context where the manager can spawn,
   inspect, message, and review engineer sessions with `talos-agent`.
2. Code plane: the engineer's assigned git worktree where code changes happen.
3. Validation plane: the engineer's worktree-local Talos environment created
...
- `talos-agent` is orchestrator-only. Engineers do not need to know about it or
  use it.
...
- All feedback to engineers goes through their Talos sessions.
...
- Do not accept toy-path validation when the product behavior is meant to hold
  up under long, realistic Talos usage.
...
- the requirement to test realistically, not just on a toy talos-path
...
```text
Follow /Users/kirilldubovitskiy/projects/talos/.agents/agents/engineer.md.
...
- Report back only through this Talos session
