...
1. Control plane: your Talos session is visible to the manager in the shared
   Talos space.
2. Code plane: your assigned git worktree.
3. Validation plane: your worktree-local Talos env started with `yarn env:up`
   from that worktree.

Being visible in the shared Talos space does not mean you should test in the
...
10. Report back in this Talos session with exact commands and clear risks.
...
- Do not treat a toy talos-path as sufficient validation when the feature is
...
- Exercise the feature under conditions closer to real Talos usage: longer
...
- Report only through your own Talos session.
