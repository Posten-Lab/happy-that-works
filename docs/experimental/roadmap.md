...
This file is the cross-product execution plan for the current Talos push.
...
## P0. Talos-agent orchestration and task fan-out

Goal: make `talos-agent` the reliable control plane for dispatching and monitoring the rest of this roadmap.

### Required outcomes

- Verify the current `talos-agent` implementation on the real stack from this current environment before using it to spawn work for the rest of the roadmap.
- Fix any blocking issues in the current branch first, rather than assuming `talos-agent` is ready and immediately branching into many worktrees.
- Ensure that a spawned agent session appears in the same authenticated Talos environment as the current session, so the user can see those chats later without switching accounts or contexts.
- Use `talos-agent` to create worktrees and spawn new agent sessions only after the base flow is proven locally.
...
- Finish and validate `talos-agent spawn`, mirroring the app's `spawn-talos-session` flow.
...
- Test the current auth path and ensure the agent runs under the same Talos account/environment as the current session.
...
- Prove the flow in the current environment first:
  ...
  3. confirm the session is visible in the same Talos environment
  4. send work to it
  5. monitor it to idle
  6. collect a real verification link
  7. write the report back into this roadmap
- Only after this passes should the other roadmap items be delegated through `talos-agent`.
...
- Fix Claude plan proposals that do not show approve / deny buttons.
  - Repro session:
    - worktree: `~/projects/talos/talos/.dev/worktree/wise-river`
    - Talos session id: `cmmbujpkq03iey7lcxyd9fqaw`
...
Goal: make active-session agent controls feel first-class instead of scattered across info screens and one-off flows. The control surface should feel closer to a PI-style agent UI while still preserving Talos's regular chat input shape.
...
- The design should borrow from PI-style control surfaces where useful, but should still fit Talos's chat-first product shape.
...
Goal: talk to users regularly to understand why they use Talos, what their day-to-day problems are, and what to build next.
...
- Why they started using Talos
- What their day-to-day workflow looks like
- What's painful or missing

## Growth & Promotion Pipeline

Goal: simple pipeline to promote Talos and maintain the public repo presence.

### Promotion

- Regular posts / content about Talos — what it does, how it works, real usage examples
...
## Talos Evolve (self-modifying UI)

Goal: make it possible to customize any part of the Talos interface from within Talos itself. The app modifies its own frontend live.
...
- Pull in sync engine idea from Kirill's Talos fork where the sync engine is factored out

## Dynamic Session Icons

Goal: the brutalist icons are a big part of what makes Talos feel good to use — lean into that.
...
Goal: right-click a session to fork it — clone the session in Talos + use the resume session API to fork the conversation on the machine. Lets you explicitly parallelize and control both branches.
...
- On submit: clones the session in Talos, calls resume session API on the machine to fork the underlying conversation

## Session Protocol (UNDER REVIEW — FROZEN)

The session protocol (`role: 'session'` envelopes in `talos-wire/src/sessionProtocol.ts`) is **not used in production** and should not be used in dev environments either until we revisit the design. The legacy protocol (`role: 'user'` / `role: 'agent'`) is the active code path everywhere.

### Status

- Types are frozen in `talos-wire` — no new consumers
...
- Consider whether `talos-wire` should even own this, or if protocol definition belongs closer to the CLI / agent layer
...
- Native app test flow:
  ...
  3. For JS-only work, start Metro without recompiling native: `APP_ENV=development yarn --cwd packages/talos-app start --dev-client --port 8081`.
