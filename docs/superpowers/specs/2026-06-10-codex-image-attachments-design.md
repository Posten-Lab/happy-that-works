...
Give Codex image attachment parity with Claude in Talos sessions. A user should be able to attach one or more images to a Codex message from the Talos app, send the message, and have Codex receive the images together with the text.

## Scope

- Reuse the existing Talos attachment pipeline: encrypted blob upload, session `file` event, and inline image rendering in chat history.
...
- Keep history, resume, and fork behavior safe: show Talos-owned `file` events when they exist, and avoid inventing image records when only an unavailable provider-local path remains.
...
- Rebuild Codex provider history as the source of truth for Talos attachments.
- Store decrypted attachment bytes in Talos server storage or session metadata.
...
Codex fork backfill currently happens only on the `TALOS_FORK_CODEX_THREAD_ID` path in `runCodex.ts`, where the runner calls `readThread` and maps provider turns through `mapCodexThreadToSessionEnvelopes`. Direct `talos codex --resume <threadId>` currently resumes the provider thread but does not replay provider turns into a new Talos session.

## Architecture

Use Talos attachments as the source of truth and Codex `localImage` items as the delivery mechanism.
...
Talos history is authoritative for images that were sent through Talos. Because the app stores `file` events before text, reopening the same Talos session should render images from those stored encrypted refs without relying on Codex provider history.

Codex thread backfill should continue to restore text, agent messages, reasoning, and tools from provider history. It should also restore provider `localImage` inputs when the local cached file still exists and the target Talos session does not already contain the matching image event.

If a Codex thread item contains provider image input during a provider-history backfill:

- If Talos already has a matching `file` event in the session stream, prefer that stored event.
- If no matching Talos `file` event exists, the provider item is `localImage`, and the file still exists locally, backfill uploads it through the normal encrypted attachment path and emits a `file` event.
...
This gives good behavior for Talos-originated sessions while avoiding false history records when provider-local files have disappeared.

Fork backfill is required because the current Codex fork flow creates a new Talos session from provider thread history. Direct `talos codex --resume <threadId>` provider-history image backfill is allowed but not required for the first implementation unless the implementation already extends that path to read provider turns. Existing Talos-session reopen and reconnect flows must continue to use stored Talos `file` events.
...
Cached plaintext image files are local to the machine running the CLI. They are not uploaded to Talos server, are not referenced from Talos server metadata, and do not include original user file names.
...
- Codex fork backfill uploads existing local-image cache files into the new Talos session and degrades safely when provider-local image paths are unavailable
