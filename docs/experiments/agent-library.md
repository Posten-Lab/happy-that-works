...
Agent definitions support Codex, Claude, and Muse Code. Model availability comes from the selected machine and runtime. Existing session snapshots are preserved. Specialties are descriptions of focus, not verified tool capabilities. Browser/vision/image-generation capability matching, tool grants, automatic delegation, workflow execution, shared agent discovery, persistent agent memory and collective planning/review are future increments. The UI makes these boundaries explicit.

Agent creation and editing offer Ask, Codex Read only, and YOLO (no approvals). The saved choice travels with new direct sessions and their immutable snapshots; editing it does not change sessions already running. YOLO also disables sandboxing in Codex and Muse. Workflow stages retain their own tool and workspace restrictions regardless of this direct-session preference.

Reference imports show reading, added-to-draft, and explicit not-attached feedback next to the file controls. Rejected files are never truncated or added. Save the agent to persist draft references.
...
- Definitions allow five instruction files, at most 64,000 characters each (256 KB maximum before reading), and 24,000 characters of main instructions. The library has a 128,000 serialized-character budget and at most 100 agents, leaving space for encrypted account settings.
