...
adapted for Talos's encrypted storage and with permissions/questions unified into
...
[`packages/talos-app/sources/sync/typesRaw.ts`](../../packages/talos-app/sources/sync/typesRaw.ts),
...
1. **Project config** — `.talos/config.json` or equivalent
...
### Talos's constraint: encrypted storage

OpenCode stores plaintext rows in SQLite and patches them freely. Talos stores
...
This matches Talos's existing message delivery model. The inner plaintext
