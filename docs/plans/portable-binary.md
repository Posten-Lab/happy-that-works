...
## Overview
Create a portable, self-contained distribution of talos-server as a single Bun-compiled binary. It runs without Redis (already has in-memory event bus), uses PGlite for embedded PostgreSQL, and local filesystem for file storage. CLI provides `talos-server migrate` and `talos-server serve` commands.
...
- [ ] Add `@electric-sql/pglite`, `pglite-prisma-adapter` to talos-server dependencies
...
- [ ] Add `build:standalone` script to `package.json`: `bun build ./sources/standalone.ts --compile --outfile talos-server`
...
- [ ] Test: `./talos-server migrate` creates and migrates a PGlite database
- [ ] Test: `./talos-server serve` starts the server with PGlite

### Task 7: Verify end-to-end
- [ ] Build the binary
- [ ] Run `./talos-server migrate` — verify database is created in `./data/`
- [ ] Run `./talos-server serve` — verify server starts, health endpoint responds
...
**CLI usage:**
```
talos-server migrate    # Apply database migrations
talos-server serve      # Start the server
```

## Post-Completion
- Document env vars for portable mode (`PGLITE_DIR`, `DATA_DIR`, `TALOS_MASTER_SECRET`)
- Test on Linux for cross-platform binary (Bun cross-compile)
- Consider adding `talos-server init` command for first-time setup
