...
Implement `docs/session-protocol.md` as the new message format for Codex sessions in the CLI, and add client-side support in `talos-app` to parse, normalize, and render these messages alongside existing legacy formats (output, codex, acp).
...
- ⚠️ `packages/talos-cli` and `packages/talos-app` do not define a `lint` script; verification used full test suites plus `yarn typecheck` in both packages.
...
- [x] Create `packages/talos-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/talos-cli/src/api/apiSession.ts`
...
- [x] Run full test suite — `yarn test` in talos-cli, talos-app
