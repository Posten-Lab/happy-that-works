# talos-agent CLI Tool

## Overview
A new standalone CLI tool (`talos-agent`) in `packages/talos-agent` that acts as a dedicated client for controlling Talos agents remotely. Unlike `talos-cli` which both runs and controls agents, `talos-agent` only controls them — listing machines, spawning sessions on a machine, creating sessions, sending messages, reading history, monitoring state, and stopping sessions.

This is a completely separate client from `talos-cli`. It has its own authentication flow (account auth via QR code, same as device linking in the mobile app), its own credential storage (`~/.talos/agent.key`), and is written from scratch with no code sharing.

## Context
- **Existing system**: Monorepo with `talos-cli` (agent runtime + control), `talos-server` (Fastify + PostgreSQL + Redis), `talos-app` (React Native mobile)
- **Server API**: REST endpoints at `https://api.cluster-fluster.com` + Socket.IO at `/v1/updates`
- **Authentication**: Uses account auth flow (`/v1/auth/account/request` + `/v1/auth/account/response`) — generates ephemeral keypair, displays QR code (`talos:///account?[base64url-publicKey]`), user scans with existing Talos mobile app to approve, receives encrypted account secret
- **Credential storage**: `~/.talos/agent.key` (separate from talos-cli's `~/.talos/access.key`)
...
- **Integration tests**: Use the real environment bootstrap (`yarn env:up:authenticated`) and exercise the live server + daemon + CLI stack. Do not use mocked acceptance coverage for `talos-agent spawn`.
...
- [x] Create `packages/talos-agent/` directory with `package.json` (name: `talos-agent`, type: module, bin: `./bin/talos-agent.mjs`)
- [x] Create `tsconfig.json` with strict mode, path aliases (`@/` → `src/`), ESM output
- [x] Create `bin/talos-agent.mjs` entry point wrapper (mirrors talos-cli pattern: spawns node with `--no-warnings`)
...
- [x] Create `src/config.ts` — reads `TALOS_SERVER_URL` (default: `https://api.cluster-fluster.com`), `TALOS_HOME_DIR` (default: `~/.talos`), derives credential file path as `${talosHomeDir}/agent.key`
- [x] Create `src/credentials.ts`:
  ...
  - `readCredentials(config)` — parses `~/.talos/agent.key` JSON `{ token, secret }`, decodes secret from base64, derives contentKeyPair via `deriveContentKeyPair(secret)`. Returns `Credentials` or `null` if file missing.
  - `writeCredentials(config, token, secret)` — writes `{ token, secret: base64(secret) }` to `~/.talos/agent.key`
  - `clearCredentials(config)` — deletes `~/.talos/agent.key`
  - `requireCredentials(config)` — calls `readCredentials`, throws with "Run `talos-agent auth login` first" if null
...
### Task 4: Authentication command (`talos-agent auth`)
- [x] Create `src/auth.ts` implementing the account auth flow:
  ...
  3. Generate QR code data: `talos:///account?` + base64url(keypair.publicKey)
  4. Display QR code in terminal using `qrcode-terminal`
  5. Print instructions: "Scan this QR code with the Talos app (Settings → Account → Link New Device)"
...
- [x] Add `talos-agent auth login` subcommand that runs the flow above
- [x] Add `talos-agent auth logout` subcommand that calls `clearCredentials()`
- [x] Add `talos-agent auth status` subcommand that reads credentials and prints auth status (authenticated / not authenticated)
...
- [x] Create `src/index.ts` using `commander` with program name `talos-agent`
- [x] `talos-agent list` — calls `listSessions`, displays table: ID (truncated), name/summary, path, status (active/inactive), last active time. With `--json` outputs raw JSON. With `--active` filters to active only.
- [x] `talos-agent status <session-id>` — fetches session via list + filter by ID prefix, connects Socket.IO to get live state, displays: session ID, metadata (path, host, lifecycle state), agent state (idle/busy, pending requests count), last message preview. With `--json` outputs raw JSON. Disconnects after displaying.
...
- [x] `talos-agent create --tag <tag> [--path <path>]` — creates new session with given tag and metadata (path defaults to cwd, host to hostname). Prints session ID. With `--json` outputs full session JSON.
- [x] `talos-agent send <session-id> <message>` — resolves session key, connects Socket.IO, sends user message (encrypted with AES-256-GCM), optionally waits for idle with `--wait`, and supports `--yolo` to send `permissionMode=yolo`. Disconnects after. Prints confirmation. With `--json` outputs message details.
...
- [x] `talos-agent history <session-id>` — fetches messages via HTTP, resolves session encryption key (dataKey or legacy), decrypts each message, displays in chronological order with role/timestamp. With `--json` outputs raw JSON. With `--limit <n>` limits output.
- [x] `talos-agent stop <session-id>` — connects Socket.IO, sends `session-end` event, disconnects. Prints confirmation.
- [x] `talos-agent wait <session-id> [--timeout <seconds>]` — connects Socket.IO, waits for agent idle state (no pending requests, not controlled by user), prints when idle or times out (default 300s). Exit code 0 on idle, 1 on timeout.
...
- [x] Verify interop: session created by talos-agent is visible and controllable from mobile app
- [x] Verify interop: session created by talos-cli can be listed and history read by talos-agent
- [x] Run full test suite (unit tests)
- [x] Run linter — all issues must be fixed

### Task 11: [Final] Update documentation
- [x] Add README.md to `packages/talos-agent/` with usage examples for all commands
- [x] Update root README if it references packages

### Task 12: Machines and spawn
- [x] Add `talos-agent machines [--active] [--json]`
- [x] Add machine record decryption using the existing account content key derivation and record data key pattern
- [x] Add `talos-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]`
- [x] Reuse the existing machine RPC contract (`spawn-talos-session`) without encryption shortcuts
- [x] Add a real integration test that boots `yarn env:up:authenticated`, authenticates `talos-agent`, lists machines, spawns via the real daemon RPC path, and verifies the live session

## Technical Details

### CLI Commands Summary
```
talos-agent auth login                          # Authenticate via QR code (scanned by Talos mobile app)
talos-agent auth logout                         # Clear stored credentials
talos-agent auth status                         # Show authentication status

talos-agent machines [--active] [--json]        # List machines
talos-agent list [--active] [--json]            # List all sessions
talos-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]  # Spawn on a machine
talos-agent status <session-id> [--json]        # Get live session state
talos-agent create --tag <tag> [--path <path>] [--json]  # Create new session
talos-agent send <session-id> <message> [--yolo] [--wait] [--json]  # Send message
talos-agent history <session-id> [--limit <n>] [--json]    # Read message history
talos-agent stop <session-id>                   # Stop a session
talos-agent wait <session-id> [--timeout <s>]   # Wait for agent to become idle
```

### Authentication Flow (Account Auth)
```
talos-agent                          Talos Server                    Talos Mobile App
     |                                    |                               |
...
### Credential File Format (`~/.talos/agent.key`)
```json
{
...
**For new sessions (created by talos-agent):**
...
**For existing sessions (created by talos-cli or other clients):**
...
## Post-Completion
**Manual verification:**
- Test full auth flow: run `talos-agent auth login`, scan QR with Talos app, verify credentials saved
...
- Test `history` command for sessions created by both `talos-agent` and `talos-cli`
- Test cross-client interop: messages from talos-agent readable by mobile app and vice versa
