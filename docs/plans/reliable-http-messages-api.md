...
⚠️ No lint script is defined in `packages/talos-server/package.json`, so linter execution is currently not available.
...
- Update `talos-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `talos-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
