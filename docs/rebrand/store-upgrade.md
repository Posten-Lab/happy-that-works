...
Targeted regressions cover the generated config, production identity override refusal, isolated nonproduction identifiers, version/build settings, existing native secure credentials and MMKV data, and both pairing-link schemes. The [actual native simulator upgrade](evidence/native-upgrade/README.md) also passed: original-source 1.7.0 was upgraded to Talos 2.0.0 without uninstalling, preserving authentication, encrypted history, and the explicit local theme. Talos sent a new message into the same session, and both messages survived native relaunch.

Those simulator builds use ad hoc signatures. The actual distributed build 14 signing identity was inspected separately; compare the final EAS artifact against it before submission. A physical-device signed update remains a separate check: install Talos over the existing app without uninstalling, verify the same account and history, and confirm the home-screen identity. Native building, store metadata changes, submission, and publication are separate release actions.
