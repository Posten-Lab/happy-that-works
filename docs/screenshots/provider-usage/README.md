...
- Null values remain unknown. Percentage windows, provider credits, currency spending and reset counts retain separate units. Claude currency amounts use the reported exponent or decimal places. Only legacy responses without that metadata fall back to Claude's older formatter: JPY/KRW/VND use the reported amount and other currencies divide minor units by 100. A past reset time does not cause Talos to invent a fresh zero-usage window.
