...
The final RPC check at **2026-09-06 20:21:31.480 UTC** exercised the actual local relay, built daemon, encrypted Socket.IO machine RPC and installed provider tools. Its sanitized results are embedded in [validation.json](./validation.json).

Codex returned **68% weekly allowance remaining**, **128.883025 credits**, **two available full-reset credits with expiry times**, and separate GPT-5.3-Codex-Spark five-hour and weekly windows at 100% remaining. A later live browser capture shows **66% remaining** because account usage changed during the task. The values were not changed for the screenshots.

Claude returned `unauthenticated`, which the live browser correctly renders as a sign-in state. **Actual signed-in Claude account percentages have not been validated.** No Claude login was available in the validation environment. Synthetic Claude windows demonstrate presentation only. The PR remains a draft pending genuine signed-in Claude validation, as required by the repository's E2E delivery rule.
...
| Evidence | Files |
| --- | --- |
| Actual provider data, desktop | [Dark](./live-desktop-dark.png), [light](./live-desktop-light.png) |
| Actual provider data, mobile viewport | [Light](./live-mobile-light.png) |
...
Account emails and host labels are replaced with neutral labels in the browser before publication. During capture, unrelated real or fixture cards are hidden to isolate the named scenario; provider quota values are not altered. Fixture cards are explicitly labeled **Design validation**. They use the same product renderer and encrypted local relay, but their values are synthetic and are **not actual provider E2E evidence**. Mobile screenshots exercise a browser viewport, not a native iOS/Android build.
...
The complete runs passed **760 app tests in 71 files**, **822 CLI unit tests in 93 files**, and **29 wire tests in 3 files**: **1,611 tests**. The final focused UI rerun passed **18 tests**; it repeats coverage and is not added to that total. CLI build and app/wire typechecks passed. These are unit-suite results; they do not substitute for the provider and browser checks above.
